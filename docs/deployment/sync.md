...
- **Device tokens** (`LifetimeMinutes`, 30 minutes) signed under the old key are refused
  ...
  not signed, so the key plays no part in it.
- **One caveat that is not about the key.** The device registry is still
  `InMemoryDeviceRegistry` — no Cosmos-backed adapter exists yet — so today *any*
  restart of the service forgets every pairing, and every device has to register
  again. A rotation restarts the service (a new revision); so does scaling back up from
  zero. Until the registry is persisted, that swamps everything above.

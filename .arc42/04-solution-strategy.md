...
| **Cloud** | C# / ASP.NET Core Minimal APIs on .NET, Azure Container Apps, Cosmos DB. Local ADR 0005 closed the open choice between Cosmos DB and PostgreSQL in favour of Cosmos, and settled the compute host on Container Apps: serverless bills per request with no floor, the aggregate is already a document, and the change feed is the sync protocol rather than a polling query plus a watermark table. |

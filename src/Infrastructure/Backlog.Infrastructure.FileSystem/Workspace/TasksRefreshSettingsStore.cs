...
/// </para>
/// </summary>
public sealed class TasksRefreshSettingsStore : ITasksRefreshSettings
{
    ...
    public TasksRefreshSettingsStore()
        : this(
            Path.Combine(
    ...
    public TasksRefreshSettingsStore(string path)
    {
    ...
    public TasksRefreshSettings Current { get; private set; }
    ...
    {
        if (seconds < TasksRefreshSettings.MinimumPollingIntervalSeconds)
        {
            return $"Check at least {TasksRefreshSettings.MinimumPollingIntervalSeconds} second apart.";
        }
    ...
    private string? Save(TasksRefreshSettings settings)
    {
    ...
    private TasksRefreshSettings Read()
    {
        try
        {
            if (!File.Exists(_path)) return new TasksRefreshSettings();

            var dto = JsonSerializer.Deserialize<RefreshSettingsDto>(File.ReadAllText(_path), JsonOptions);
            if (dto is null) return new TasksRefreshSettings();

            return Normalize(new TasksRefreshSettings
            {
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or JsonException)
        {
            ...
            return new TasksRefreshSettings();
        }
    }
    ...
    private static TasksRefreshSettings Normalize(TasksRefreshSettings settings) =>
        settings.PollingIntervalSeconds >= TasksRefreshSettings.MinimumPollingIntervalSeconds
            ? settings
            : settings with { PollingIntervalSeconds = TasksRefreshSettings.MinimumPollingIntervalSeconds };

    private sealed class RefreshSettingsDto
    {
        public bool PollingEnabled { get; init; } = true;

        public int PollingIntervalSeconds { get; init; } = TasksRefreshSettings.DefaultPollingIntervalSeconds;
    }
}

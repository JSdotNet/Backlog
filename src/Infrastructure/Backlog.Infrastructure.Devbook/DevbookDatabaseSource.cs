...
/// — because <c>IDevbookFolderSource</c> resolves an area per registered
/// repository, while the database is one file at the repository root above them
/// all. <see cref="DevbookDatabaseLocation"/> does that one step up; this does
...
/// </summary>
public static class DevbookDatabaseSource
{
    public static DevbookDatabase? TryOpen(IDevbookFolderSource folders, string? repositoryAlias)
    {
        ...
        foreach (var setting in folders.Folders(repositoryAlias))
        {
            ...
            if (DevbookDatabase.TryOpenForFolder(path) is { } database) return database;
        }

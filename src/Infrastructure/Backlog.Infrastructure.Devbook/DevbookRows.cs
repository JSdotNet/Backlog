...
/// <see cref="DevbookScopeProjection"/>, which decides that per scope.</param>
public sealed record DevbookNodeRow(
    string Id,
    string Type,
    string? Label,
    string? Folder,
...
public sealed record DevbookEdgeRow(string Id, string Type, string Source, string Target);

/// <summary>One row of <c>node_attribute</c> — a list-valued field of a
/// <c>meta</c> block flattened to one row per value.</summary>
public sealed record DevbookNodeAttributeRow(string NodeId, string Name, string Value);
...
public sealed record DevbookOutlineRow(
    long Id,
    string Scope,
    long? ParentId,
    int Ordinal,
...
public sealed record DevbookArchifyArtifactRow(
    string ChapterPath,
    string FenceHash,
    int Ordinal,
    string? Type,

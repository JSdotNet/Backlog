...
/// one.</summary>
public static class DevbookEmbeddingModel
{
    /// <summary>
    ...
    /// ignored — see <see cref="DevbookSemanticSearch"/>.</para>
    /// </summary>
    public const string Default = "text-embedding-3-small";
}

/// <summary>
/// <see cref="IDevbookVectorSearch"/> over <c>chapter_embedding</c>, by
/// brute-force cosine.
///
...
/// with the same sentence <see cref="DevbookFullTextSearch"/> uses, because it
...
public sealed class DevbookSemanticSearch(IDevbookFolderSource folders, string? model = null) : IDevbookVectorSearch
{
    private const string Surface = "Search by meaning";

    private readonly string _model = string.IsNullOrWhiteSpace(model) ? DevbookEmbeddingModel.Default : model;

    /// <summary>The model this reader compares against, for a caller that has to
    /// embed the question with the same one.</summary>
    public string Model => _model;

    public DevbookSearchAnswer Nearest(
        ...
        int limit = 30)
    {
        ...
        if (queryVector.Count == 0 || limit <= 0) return DevbookSearchAnswer.None;
        ...
        var hits = database.Embeddings(_model, scope)
            .Select(row => new DevbookSearchHit(
                ...
                string.Empty,
                DevbookVectorMath.Cosine(query, AsSpan(row.Vector))))

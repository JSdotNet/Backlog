...
/// </summary>
public sealed partial class DevbookDatabase
{
    /// <summary>
    ...
    /// typing — see <see cref="DevbookSearchExpression"/>, which is the only
    ...
    /// matched either. See <c>tools/devbook/devbook-schema.mjs</c>.</para>
    ///
    ...
    public IReadOnlyList<DevbookSearchRow> SearchChapters(string? match, string? scope, int limit)
    {
    ...
    /// </summary>
    public IReadOnlyList<DevbookEmbeddingRow> Embeddings(string? model, string? scope)
    {
        ...
        return Query(
            ...
            reader => new DevbookEmbeddingRow(
                reader.GetString(0),
                Text(reader, 1),
                reader.GetString(2),
                Text(reader, 3),
    ...
    /// <c>tools/devbook/devbook-schema.mjs</c> beside the table, because a
...
public sealed record DevbookSearchRow(
    string Path,
    string? Folder,
    string Slug,
    string? Title,
...
public sealed record DevbookEmbeddingRow(
    string Path,
    string? Folder,
    string Slug,
    string? Title,

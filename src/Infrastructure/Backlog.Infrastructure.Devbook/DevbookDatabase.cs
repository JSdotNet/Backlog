...
/// <see cref="DevbookRetrievalTier"/>.</para>
///
...
/// <c>DevbookDatabase.Retrieval.cs</c> rather than lengthening this file. They
...
/// </remarks>
public sealed partial class DevbookDatabase : IDisposable
{
    private readonly SqliteConnection _connection;
    private readonly IReadOnlyDictionary<string, string> _meta;

    private DevbookDatabase(string databasePath, SqliteConnection connection, IReadOnlyDictionary<string, string> meta)
    {
    ...
    /// </summary>
    public static DevbookDatabase? TryOpenForFolder(string? devbookFolderPath) =>
        TryOpen(DevbookDatabaseLocation.ForDevbookFolder(devbookFolderPath));
    ...
    /// </summary>
    public static DevbookDatabase? TryOpen(string? databasePath)
    {
        ...
        try
        {
            ...
            if (!meta.TryGetValue(DevbookDatabaseSchema.SchemaVersionKey, out var declared)
                || !int.TryParse(declared, NumberStyles.Integer, CultureInfo.InvariantCulture, out var version)
                || version != DevbookDatabaseSchema.Version)
            {
                connection.Dispose();
                return null;
            }

            return new DevbookDatabase(databasePath, connection, meta);
        }
    ...
    public DateTime GeneratedUtc =>
        _meta.TryGetValue(DevbookDatabaseSchema.GeneratedAtKey, out var value)
        && DateTime.TryParse(
            value,
    ...
    public string? EmbeddingModel =>
        _meta.TryGetValue(DevbookDatabaseSchema.EmbeddingModelKey, out var value) && value.Length > 0 ? value : null;
    ...
    public DevbookRetrievalTier Retrieval => DevbookRetrieval.TierFor(this);

    /// <summary>
    /// The resolved outline of one scope — <c>.domain</c>, or
    /// <see cref="DevbookDatabaseSchema.RepositoryScope"/> for the whole
    ...
    /// the tree from <see cref="DevbookOutlineRow.ParentId"/> and never sorts.
    /// </summary>
    public IReadOnlyList<DevbookOutlineRow> Outline(string scope)
    {
    ...
    /// </summary>
    public IReadOnlyDictionary<string, DevbookFileState> FileStates(string scope)
    {
        var rows = Query(
            ScopeFilter("SELECT path, size, mtime, source_hash FROM chapter", scope, "path"),
            command => AddScope(command, scope),
            reader => new DevbookFileState(reader.GetString(0), reader.GetInt64(1), reader.GetInt64(2), reader.GetString(3)));

        var states = new Dictionary<string, DevbookFileState>(StringComparer.OrdinalIgnoreCase);
    ...
    /// <see cref="DevbookScopeProjection"/> for the scoped reading.</summary>
    public IReadOnlyList<DevbookNodeRow> Nodes() =>
        Query(
            """
    ...
    public IReadOnlyList<DevbookEdgeRow> Edges() =>
        Query(
            "SELECT id, type, source, target FROM edge ORDER BY rowid",
            _ => { },
            reader => new DevbookEdgeRow(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3)));
    ...
    /// them.</param>
    public IReadOnlyList<DevbookArchifyArtifactRow> ArchifyArtifacts(string directory)
    {
        ...
        var rows = Query(
            ...
            reader => new DevbookArchifyArtifactRow(
                reader.GetString(0),
                reader.GetString(1),
                reader.GetInt32(2),
                Text(reader, 3),
    ...
    public IReadOnlyList<DevbookProblemRow> Problems(string? scope = null) =>
        Query(
            scope is null
            ...
            reader => new DevbookProblemRow(reader.GetString(0), reader.GetString(1), Text(reader, 2), reader.GetString(3)));
    ...
    private static bool IsRepositoryScope(string? scope) =>
        string.IsNullOrWhiteSpace(scope) || scope == DevbookDatabaseSchema.RepositoryScope;

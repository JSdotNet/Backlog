...
/// <see cref="DevbookNodeRow.Folder"/>. They agree for every chapter in a
...
/// </summary>
public sealed record DevbookScopeProjection(
    string Scope,
    IReadOnlyList<DevbookNodeRow> Nodes,
    IReadOnlyList<DevbookEdgeRow> Edges)
{
    /// <summary>
    /// Projects the whole graph onto one scope. The repository scope
    /// (<see cref="DevbookDatabaseSchema.RepositoryScope"/>) keeps everything,
    /// which is what makes the whole-repository atlas a case of this rather than an
    /// exception to it.
    /// </summary>
    public static DevbookScopeProjection Project(
        string scope,
        IReadOnlyList<DevbookNodeRow> nodes,
        IReadOnlyList<DevbookEdgeRow> edges)
    {
        ...
        if (string.IsNullOrWhiteSpace(scope) || scope == DevbookDatabaseSchema.RepositoryScope)
        {
        ...
        var boundary = new Dictionary<string, DevbookNodeRow>(StringComparer.Ordinal);
        ...
        var projected = new List<DevbookNodeRow>(kept.Count + boundary.Count);
        foreach (var node in nodes)
        {
        ...
        return new DevbookScopeProjection(scope, projected, projectedEdges);
    }
}

...
/// (local ADR 0007; the <c>+</c> sigil is what the parser files a plan tag
/// under, and the picker offers the same), <c>id:</c>/<c>after:</c> for order,
/// <c>!draft</c> on
...
public static class AzureFoundryPlanPrompt
{
    ...
    public const string Text = Marker + """
        ...
        1. `# Title` — one line. Do not put a `+tag`, a `#tag` or an `@name` in the title.
        ...
        - The plan tag, `+<plan tag>` with the value given under "Plan tag:", on every entry — the `+` sigil, never `#`. It is the plan's whole identity; an entry without it cannot be recognised on a re-import.

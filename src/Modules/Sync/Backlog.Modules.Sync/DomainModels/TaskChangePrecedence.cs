...
/// <summary>
/// Whether a pushed copy of a task document may replace the one the replica
/// holds.
/// <para>
/// <b>A push may never move a document backwards.</b> Every device pushes
/// everything above its own watermark, and a document it <em>received</em> sits
/// above that watermark exactly as one it edited does — so each device echoes
/// every document it pulled, once, on its next push. Under a blind upsert that
/// echo replaced whatever the replica had learned in between: a tombstone one
...
/// version twice and changes nothing. What this costs is the genuine race the
/// ADR ordered by arrival: two machines editing one task in the same interval
...
/// </para>
/// <para>
/// Shared by both replica adapters so the in-memory store every endpoint test
/// runs against and the Cosmos store the service deploys with cannot disagree
/// about what a stale push is.
/// </para>
/// </summary>
public static class TaskChangePrecedence
{
    ...
    public static bool Supersedes(TaskChange inbound, TaskChange stored)
    {

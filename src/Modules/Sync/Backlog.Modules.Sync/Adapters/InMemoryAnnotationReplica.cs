...
public sealed class InMemoryAnnotationReplica : IAnnotationReplica
{
    ...
    private sealed class OwnerPartition
    {
        ...
        internal bool Write(AnnotationChange change, Guid deviceId)
        {
            lock (_gate)
            {
                ...
                // change. AnnotationChangePrecedence says what stale means.
                if (_documents.TryGetValue(change.Id, out var held)
                    && !AnnotationChangePrecedence.Supersedes(change, held.Change))
                {
                    return false;
                }

                // Whole-document last-write-wins among the versions that get
                // this far: re-writing a document moves it to the end of the
                // feed, exactly as a Cosmos upsert restamps `_ts`.

...
/// </summary>
/// <para>
/// It asks its own module's store, not Second Brain's folder source, even though
/// <c>.backlog</c> is one of the configured knowledge folders and the resolution
/// is literally the same. Reaching across would make Backlog Management depend on
/// a context <c>.domain/context-map.md</c> calls a Partnership; the adapter behind
/// <see cref="IBacklogStore"/> holds that join instead.
/// </para>
public sealed class RepositoryBacklogSource(IBacklogStore store)
{
    ...
    {
        var location = store.ResolveBacklogFolder(repositoryAlias);
        if (!location.Available || location.FullPath is null) return [];

        var root = location.RepositoryRootPath is { Length: > 0 } clone ? clone : location.FullPath;
        ...
        foreach (var file in files)
        {
            ...
            for (var segmentIndex = 0; segmentIndex < entries.Count; segmentIndex++)
            {
                ...
                documents.Add(new RepositoryBacklogDocument(
                    entry.RawText,
                    entry.Status,
                    location.RepositoryFullName ?? "Configured repository",
                    relativePath,

...
namespace Backlog.Modules.Backlog.Features.ReorderEntries;

/// <summary>Writes a new hand-made ranking: position in the list becomes the
/// entry's order.</summary>
public sealed record ReorderEntriesCommand(IReadOnlyList<Guid> IdsInOrder);

public sealed class ReorderEntriesCommandHandler(IBacklogRepository entries)
    : ICommandHandler<ReorderEntriesCommand>
{
    public async Task Handle(ReorderEntriesCommand command, CancellationToken cancellationToken = default)
    {

...
public sealed record DevbookChapterAddress(string Path, string Slug, string? Title, string? Folder)
{
...
public sealed record DevbookSearchHit(DevbookChapterAddress Chapter, string Excerpt, double Score);
...
/// the reader's word — <c>DevbookRetrievalTier</c>, in
/// <c>Backlog.Infrastructure.Devbook</c>. What a caller of this port needs is
/// the answer and, when there is none, the sentence to show.</para>
/// </summary>
public sealed record DevbookSearchAnswer(IReadOnlyList<DevbookSearchHit> Hits, string? UnavailableMessage)
{
    /// <summary>Nothing matched, which is a real and ordinary answer.</summary>
    public static DevbookSearchAnswer None { get; } = new([], null);

    /// <summary>There was nothing to search, which is not.</summary>
    public static DevbookSearchAnswer Unavailable(string message) => new([], message);

    public static DevbookSearchAnswer For(IReadOnlyList<DevbookSearchHit> hits) => new(hits, null);
...
/// </summary>
public interface IDevbookSearch
{
    /// <summary>
    ...
    DevbookSearchAnswer Search(string? query, string? repositoryAlias = null, string? scope = null, int limit = 30);
}

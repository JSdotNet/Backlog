...
/// </summary>
internal static class DevbookReadingOrder
{
    ...
    /// names, or <see cref="DevbookFolderReadingOrder.Empty"/> when the folder
    /// declares none this reader can read.
    /// </summary>
    public static DevbookFolderReadingOrder Read(string folderPath)
    {
        if (string.IsNullOrWhiteSpace(folderPath)) return DevbookFolderReadingOrder.Empty;

        var orderPath = Path.Combine(folderPath, FileName);
        if (!File.Exists(orderPath)) return DevbookFolderReadingOrder.Empty;
        ...
        try
        {
            ...
            if (root.ValueKind != JsonValueKind.Object) return DevbookFolderReadingOrder.Empty;

            if (!root.TryGetProperty("version", out var version)) return DevbookFolderReadingOrder.Empty;
            if (version.ValueKind != JsonValueKind.Number) return DevbookFolderReadingOrder.Empty;
            if (!version.TryGetInt32(out var declaredVersion)) return DevbookFolderReadingOrder.Empty;
            if (declaredVersion != SupportedVersion) return DevbookFolderReadingOrder.Empty;
            ...
            if (!root.TryGetProperty("scope", out var scope)) return DevbookFolderReadingOrder.Empty;
            if (scope.ValueKind != JsonValueKind.String) return DevbookFolderReadingOrder.Empty;
            if (scope.GetString() is not { Length: > 0 } key) return DevbookFolderReadingOrder.Empty;

            if (!root.TryGetProperty("directories", out var directories)) return DevbookFolderReadingOrder.Empty;
            if (directories.ValueKind != JsonValueKind.Object) return DevbookFolderReadingOrder.Empty;
            if (!directories.TryGetProperty(key, out _)) return DevbookFolderReadingOrder.Empty;

            var declared = new Dictionary<string, DevbookDeclaredDirectory>(StringComparer.OrdinalIgnoreCase);
            ...
            return new DevbookFolderReadingOrder(declared);
        }
        catch (JsonException)
        {
            return DevbookFolderReadingOrder.Empty;
        }
        catch (IOException)
        {
            return DevbookFolderReadingOrder.Empty;
    ...
    private static DevbookDeclaredDirectory ReadDirectory(JsonElement declared)
    {
        ...
        return new DevbookDeclaredDirectory(rootDocument, names);
    }
}
...
/// </summary>
internal sealed class DevbookFolderReadingOrder(IReadOnlyDictionary<string, DevbookDeclaredDirectory> directories)
{
    ...
    /// sort.</summary>
    public static DevbookFolderReadingOrder Empty { get; } =
        new(new Dictionary<string, DevbookDeclaredDirectory>(StringComparer.OrdinalIgnoreCase));
...
internal sealed record DevbookDeclaredDirectory(string? RootDocument, IReadOnlyList<string> Names);

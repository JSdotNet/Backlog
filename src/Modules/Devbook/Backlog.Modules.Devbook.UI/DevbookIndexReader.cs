...
/// back is the same shape either way, which is the point: the rest of Devbook
/// does not know which rung answered.
/// </para>
/// <para>
...
/// </para>
/// </summary>
public sealed class DevbookIndexDocument
{
    private readonly IReadOnlyDictionary<string, DevbookFileState>? _fileStates;

    private DevbookIndexDocument(
        string folderPath,
        DateTime writtenUtc,
        IReadOnlyList<DevbookIndexEntry> entries,
        IReadOnlyDictionary<string, DevbookFileState>? fileStates = null)
    {
    ...
    public IReadOnlyList<DevbookIndexEntry> Entries { get; }

    /// <summary>Every file entry in the outline, depth-first, in reading order.</summary>
    public IEnumerable<DevbookIndexEntry> Files => Flatten(Entries).Where(entry => entry.IsFile);

    /// <summary>The directory entries at the top of the outline — one per bounded context, in <c>.domain</c>.</summary>
    public IEnumerable<DevbookIndexEntry> Directories => Entries.Where(entry => entry.IsDirectory);
    ...
    /// is tried next.</para>
    /// </summary>
    private static DevbookIndexDocument? TryReadDatabase(string folderPath)
    {
        ...
        return new DevbookIndexDocument(
            folderPath,
    ...
    {
        var top = new List<DevbookIndexEntry>();
        var byId = new Dictionary<long, DevbookIndexEntry>(rows.Count);

        foreach (var row in rows)
        {
            var entry = new DevbookIndexEntry
            {
                Type = row.Type,
                Name = row.Name,
                Path = row.Path,
    ...
    private static DevbookIndexDocument? TryReadJson(string folderPath)
    {
        ...
        try
        {
            ...
            // see the consumer rules in devbook-derived-artifacts.
            if (!SupportedSchemaVersions.Contains(payload.SchemaVersion)) return null;

            return new DevbookIndexDocument(folderPath, File.GetLastWriteTimeUtc(indexPath), payload.Entries);
        }
    ...
    /// </summary>
    public bool IsStale(DevbookIndexEntry entry)
    {
    ...
    /// </summary>
    public bool LooksStale(DevbookIndexEntry entry)
    {
        var fullPath = FullPath(entry);
    ...
    public bool Exists(DevbookIndexEntry entry) => File.Exists(FullPath(entry));
    ...
    /// <c>1</c> while the current <c>devbook-base</c> plugin emits <c>4</c>,
    ...
    private sealed class IndexPayload
    {
        ...
        public List<DevbookIndexEntry>? Entries { get; set; }
    }
}
...
/// </summary>
public sealed class DevbookIndexEntry
{
    ...
    public List<DevbookIndexEntry>? Children { get; set; }
    ...
    public DevbookIndexEntry? RootDocument =>

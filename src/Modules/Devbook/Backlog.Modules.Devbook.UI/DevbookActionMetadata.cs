...
public sealed record DevbookIssueLink(string RepoFullName, int IssueNumber, string? LabelOverride = null)
{
    public string Url => $"https://github.com/{RepoFullName}/issues/{IssueNumber}";
    public string Label => LabelOverride ?? $"#{IssueNumber}";
}

public sealed record DevbookActionItem(
    string Title,
    string Kind,
    string Path,
...
public static partial class DevbookActionMetadata
{
    private static readonly string[] IssueKeys = ["devbook-issue", "github-issue", "issue"];
    ...
    public static DevbookIssueLink? Issue(IReadOnlyDictionary<string, string> metadata, string? defaultRepository)
    {
    ...
    public static string BuildPrompt(DevbookActionItem item) =>
    ...
    private static bool TryParseIssue(string? raw, string? defaultRepository, out DevbookIssueLink link)
    {
        link = default!;
        ...
        if (match.Success && int.TryParse(match.Groups["number"].Value, out var urlNumber))
        {
            ...
            link = new DevbookIssueLink(repo, urlNumber, $"{repo}#{urlNumber}");
            return true;
        }
        ...
        {
            var repo = $"{match.Groups["owner"].Value}/{match.Groups["repo"].Value}";
            link = new DevbookIssueLink(repo, repoNumber, $"{repo}#{repoNumber}");
            return true;
        }
        ...
        {
            link = new DevbookIssueLink(defaultRepository, number);
            return true;
        }

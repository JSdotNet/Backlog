...
public sealed record TechnologyDevbookDiagram(string Title, string Language, string Source);
...
public sealed record DevbookMetadata(
    ...
    IReadOnlyList<string> Alternatives)
{
    public static DevbookMetadata Empty { get; } = new(null, null, null, [], [], []);
}

internal static class TechnologyDevbookReader
{
    ...
    internal static TechnologyDevbookView Read(DevbookFolderLocation location, Cache? cache)
    {
        ...
        {
            return TechnologyDevbookView.Unavailable(location with
            {
        ...
        IReadOnlyDictionary<string, TechnologyBoundaryNode> boundary)
    {
        ...
        var techStatuses = DevbookStatus.Vocabulary(DevbookFolder.Tech);
        ...
        foreach (var relationship in relationships)
        {
            ...
            var folder = DevbookFolders.FromPath(relationship.ToId);
            ...
            var graphNode = new TechnologyGraphNode(
                ...
                0,
                status.Length == 0 ? string.Empty : DevbookStatus.Vocabulary(folder).SlugFor(status),
    ...
    /// <see cref="DevbookScopeProjection"/> is the reading-side restatement of the
    ...
    /// </para>
    /// </summary>
    private static TechnologyDevbookIndex? ReadDatabaseIndex(string folderPath)
    {
        using var database = DevbookDatabase.TryOpenForFolder(folderPath);
        if (database is null) return null;

        var projection = DevbookScopeProjection.Project(TechnologyScope, database.Nodes(), database.Edges());
        ...
        return new TechnologyDevbookIndex(
            new TechnologyGraphStats(projection.Nodes.Count, projection.Edges.Count, statuses),
            boundary.Count == 0 ? TechnologyDevbookIndex.NoBoundary : boundary);
    }
    ...
    /// </para>
    /// </summary>
    private static TechnologyDevbookIndex ReadJsonIndex(string path)
    {
        if (!File.Exists(path)) return TechnologyDevbookIndex.Empty;
        ...
        try
        {
            ...
            return new TechnologyDevbookIndex(ReadStats(root), ReadBoundary(root));
        }
        catch (Exception exception) when (exception is JsonException or IOException or UnauthorizedAccessException)
        {
            return TechnologyDevbookIndex.Empty;
        }
    }
    ...
    {
        if (!root.TryGetProperty("elements", out var elements)) return TechnologyDevbookIndex.NoBoundary;
        if (!elements.TryGetProperty("nodes", out var nodes) || nodes.ValueKind != JsonValueKind.Array)
        {
            return TechnologyDevbookIndex.NoBoundary;
        }
...
/// disagree.</summary>
internal sealed record TechnologyDevbookIndex(
    TechnologyGraphStats Stats,
    IReadOnlyDictionary<string, TechnologyBoundaryNode> Boundary)
{
    ...
    public static TechnologyDevbookIndex Empty { get; } = new(TechnologyGraphStats.Empty, NoBoundary);
}
...
internal sealed record TechnologyMarkdownChapter(string Title, DevbookMetadata Metadata, string Summary);
...
internal static class TechnologyMarkdownParser
{
    public static TechnologyMarkdownDocument Parse(string path, string markdown)
    {
        ...
        var metadata = DevbookMetadata.Empty;
        var summary = new List<string>();
        var chapters = new List<TechnologyMarkdownChapter>();
        var diagrams = new List<TechnologyDevbookDiagram>();
        ...
        for (var index = 0; index < lines.Length; index++)
        {
            ...
            if (marker.StartsWith("```", StringComparison.Ordinal))
            {
                ...
                {
                    diagrams.Add(new TechnologyDevbookDiagram(title.Length == 0 ? Path.GetFileName(path) : title, language, block));
                }
    ...
    private static DevbookMetadata ParseMetadata(string block)
    {

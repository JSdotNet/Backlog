...
public sealed class ProcessCopilotCliLauncher : ICopilotCliLauncher
{
    ...
    public Task LaunchAsync(CopilotCliRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new CopilotCliException("There is no backlog item prompt to send to GitHub Copilot CLI.");
        }
...
internal sealed class UnavailableCopilotCliLauncher : ICopilotCliLauncher
{
    public Task LaunchAsync(CopilotCliRequest request, CancellationToken cancellationToken = default) =>

...
/// </summary>
public sealed record DevbookReference
{
    private DevbookReference(string raw, string path, string? slug)
    {
        Raw = raw;
        Path = path;
        Slug = slug;
        Folder = DevbookFolders.FromPath(path);
    ...
    public DevbookFolder Folder { get; }
    ...
    /// </summary>
    public static DevbookReference? Parse(string? raw)
    {
        ...
        if (hash < 0) return new DevbookReference(text, text, null);
        ...
        return new DevbookReference(text, path, slug.Length == 0 ? null : slug);
    }
    ...
    /// </summary>
    public static DevbookReference? ParseDevbookPath(string? raw)
    {
        if (Parse(raw) is not { } reference) return null;
        if (reference.Folder is DevbookFolder.Unknown) return null;
    ...
    /// is a <see cref="DevbookReference"/>, and the three things it refuses are
    /// the three <see cref="ParseDevbookPath(string?)"/> already refuses — a
    ...
    public static DevbookReference? ParseDevbookPath(string? raw, string? fromDocument)
    {
        if (string.IsNullOrWhiteSpace(raw)) return null;
    ...
    private static string? Resolve(string target, string? fromDocument)
    {
        ...
        if (DevbookFolders.FromPath(forward) is not DevbookFolder.Unknown) return Normalize(forward);
    ...
    /// <see cref="DevbookFolders.FromPath"/> and <see cref="Normalize"/> already
    ...
    /// leniency <see cref="DevbookFolders.FromPath"/> already grants, because a
    ...
    public static bool TryParse(string? raw, out DevbookReference? reference)
    {
        reference = Parse(raw);

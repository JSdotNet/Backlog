...
///
/// <para>A status has two forms — the read-only <see cref="DevbookStatusPill"/>
...
/// </summary>
internal static class DevbookStatusBadge
{
    ...
    public static string Slug(DevbookFolder folder, string? status) =>
        DevbookStatus.Vocabulary(folder).SlugFor(status);
    ...
    public static bool IsUnrecognised(DevbookFolder folder, string? status) =>
        DevbookStatus.Vocabulary(folder).IsUnrecognised(status);
}

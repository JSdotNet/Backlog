...
/// </summary>
public static class DevbookInlineTargets
{
    /// <summary>
        ...
        EventCallback<DevbookReference> onNavigate)
    {
        ...
        var reference = kind is MarkdownInlineKind.Link
            ? DevbookReference.ParseDevbookPath(target, documentPath)
            : DevbookReference.ParseDevbookPath(target);
    ...
    private static RenderFragment Link(
        DevbookReference reference,
        string? text,
        string? href,
        EventCallback<DevbookReference> onNavigate) => builder =>
    {
        builder.OpenComponent<DevbookReferenceLink>(0);
        builder.AddComponentParameter(1, nameof(DevbookReferenceLink.Reference), reference);
        builder.AddComponentParameter(2, nameof(DevbookReferenceLink.Text), text);
        builder.AddComponentParameter(3, nameof(DevbookReferenceLink.Href), href);
        builder.AddComponentParameter(4, nameof(DevbookReferenceLink.OnNavigate), onNavigate);

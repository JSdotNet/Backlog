...
| [Sessions](../sessions/domain.md#session-log) | Customer/Supplier (Sessions = supplier) | Infrastructure adapter reads the Session Log directly (`AgentSessionAssistantSessionSource` maps `IAgentSessionSource` onto this context's `IAssistantSessionSource`) | `.domain/sessions/domain.md#session-log` | Productivity's Sessions part needs machine id, machine name, assistant, started at, last activity, the sources it could not read, and whether the read reached the horizon it asked for, to show what the agents have been doing — read since a horizon rather than as the capped list, so a count is never the session limit — a synchronous read of session facts, not a subscribed event. |

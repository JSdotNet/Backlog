...
| [Monitoring](../monitoring/domain.md#aggregate-progress-signal) | OHS + Published Language (Dev PC Management = supplier) | Emits `MachineStatusChanged` / `ComplianceUpdated` | `.domain/dev-pc-management/domain.md#domain-event-machinestatuschanged`, `.domain/dev-pc-management/domain.md#domain-event-complianceupdated` | Machine status, compliance, and uptime feed dashboards. |
| [Backlog](../backlog/domain.md#aggregate-backlog-entry) | Customer/Supplier (Dev PC Management = customer) | Id reference to work items | `.domain/backlog/naming.md#term-backlog-entry` | Compliance gaps can drive update tasks. |
...
| [Monitoring](../monitoring/domain.md#aggregate-progress-signal) | OHS + Published Language (Dev PC Management = supplier) | Subscribes to machine/compliance signals | `.domain/dev-pc-management/domain.md#domain-event-machinestatuschanged`, `.domain/dev-pc-management/domain.md#domain-event-complianceupdated` | Relies on infrastructure dashboard signals from this context. |
| [Sessions](../sessions/domain.md#aggregate-session-log) | Customer/Supplier (Dev PC Management = supplier) | Machine-name lookup | `.domain/dev-pc-management/domain.md#aggregate-machine-registry` | Relies on the registry to say which registered machine an `Environment` corresponds to. Not built: a locally read session is stamped by the environment that read it, so nothing is asked of the registry yet. This context took the session subject from here — see `.domain/sessions/dependencies.md`. |
...
- Sessions are not this context's subject. The `Copilot Session Tracking` service,
  ...
  feature moved to [Sessions](../sessions/domain.md#aggregate-session-log) when a

...
- `repo_ids` is a plain list of repository identifiers, not object references —
  Tasks stays decoupled from Repository Management. Projection turns each
  `repo_id` into a `ProjectionRef` when the task starts work.
- `area` and `order` are plain scalars on the root, not separate value objects —
  they place the task in the person's own working set (self-chosen grouping,
  manual rank) rather than describing the task's business state, so they carry
...
- `depends_on` is the one self-association on the diagram, and it is drawn dashed
  ...
  `Task` is its own aggregate root, so a dependency crosses an aggregate
...
- `Readiness` is attached with a dashed dependency, not an association, because
  ...
  tasks those ids resolve to on every read. `Readiness` and `TaskStatus` are
  orthogonal: status is the recorded lifecycle state, readiness is a conclusion
  about whether the task can be started, and `done` is the one value they share

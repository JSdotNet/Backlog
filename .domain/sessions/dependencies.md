...
| Dev PC Management | Customer/Supplier (Sessions = customer) | Machine-name lookup against the machine registry | `.domain/dev-pc-management/domain.md#machine-registry` | To say which registered machine an `Environment` corresponds to, when the two name the same box. **Not built.** A locally read session is stamped with the product-wide device identity (id and name) issued by the shared kernel, not with the name of the environment that read it; a registry lookup would only correlate that device id with a registered Machine, and nothing is asked of the registry today. The dependency becomes real the first time sessions arrive from an environment other than the one reading them, which local ADR 0005 now names as the point session records start replicating between the person's machines. |
...
|---|---|---|---|---|
| Productivity | Customer/Supplier (Sessions = supplier) | Infrastructure adapter reads the Session Log directly (`AgentSessionAssistantSessionSource` maps `IAgentSessionSource` onto Productivity's `IAssistantSessionSource`) | `.domain/sessions/domain.md#session-log` | AI-assisted work needs to know which agent worked where and for how long. **Built.** The adapter supplies machine id, machine name, assistant, started at, last activity, the sources it could not read, and the per-assistant cap already applied — a synchronous read of session facts, not a published event. |

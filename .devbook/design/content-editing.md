...
| `#tags` | Recognized inline per `.devbook/arc42/08-crosscutting-concepts.md#tagging-and-organization`; rendered as a subtle chip but stored as literal `#tag` text. |
...
| `+` | plan tag | `` `+release-q4` `` — the tag that files the entry under a roadmap item, stored with its sigil (`.devbook/domain/tasks/domain.md#roadmap-tag`) |
...
`task` `*high` `!ready` `@repos` `#deploy` `due:2026-08-21` `remind:2026-08-21T09:00` `after:a1b2c3` `repo:specmanager`
```
...
`task` `@repos` `id:add-command` `repo:specmanager`

# Wire the export command into the toolbar

`task` `@repos` `after:add-command` `repo:specmanager`
```
...
| A local id bridges an unsaved batch | `id:` names an entry before it has a real `task_id`, which is what every entry in a fresh paste is — none of them saved yet. A sibling's `after:` resolves against a same-document `id:` first, so one new entry can depend on another before either exists; failing that, against the `id:` a stored entry was imported under — the same plan's first, then a plan the entry is tagged with, then any entry when only one carries the id — so a plan brought in over two sittings still chains; failing that too, `after:` means a real id as it always did. An entry with no `id:` is addressable exactly as today, once saved. |
...
| A repo name repeats, like a dependency | `repo:` may appear more than once, the same as `after:` and for the same reason — an entry can target several repositories at once, and order carries no meaning. This matches the aggregate's own `repo_ids[]` (`.devbook/domain/tasks/features.md#multi-repo-targeting`), which already holds more than one. |
| A repo name is resolved, never invented | `repo:` names a repository the way a person or a pasted document actually can — by name — and is resolved against the Repository Registry (`.devbook/domain/repository-management/domain.md#repository-registry`) to the `repo_id` the aggregate stores, because nothing hand-typed can know that id. A name the registry does not recognize is simply unresolved, on the same terms as an unrecognized sigil value; this token never registers a repository on its own. Only the narrower import policy (`.devbook/domain/tasks/features.md#repository-resolution-on-import`) auto-registers an unknown name, and only for that feature. |
| An effort is a count of points | `effort:` takes a non-negative whole number of story points. `0` is a real estimate that contributes nothing; a negative or fractional value is not an estimate and is not read (`.devbook/domain/tasks/domain.md#task`). Absent means unestimated, which is a normal state and never rewritten as `0`. |
| A `plan` entry reads a narrower line | On an entry whose type word is `plan` (`#structured-metadata-sigils`) only `+tag`, `id:`, `after:`, `repo:`, `*priority` and `due:` mean anything, and they mean it about a roadmap item: the tag becomes the item's, `after:` a roadmap dependency, `due:` the window's end. **`effort:` is not honoured on a `plan` entry** — a plan owns no estimate; its size is the effort the tasks under it registered — and Import says so once when it meets one. There is no start-date token: the importer places the item. Every other token on the line describes a task and is ignored. The mapping is `.devbook/arc42/adr/0013-imported-plan-is-a-roadmap-item-laid-out-by-import.md`. |
...
| Refused status is explained | If the typed status is not a legal next step in the entry's lifecycle (`.devbook/domain/tasks/flow.md#task-lifecycle`), the hint MUST show the status that will actually be kept plus which statuses are legal next steps — the typed word is never silently dropped. |
...
| Never invented | A status is never invented: none is defaulted to the first word the folder allows, and no empty row is drawn where a record would have been. Absent means absent (`README.md#status-vocabulary`). What that looks like depends on whether the folder lets a chapter state none. Where a status is required, a block stating nothing MUST draw nothing at either level. Where stating none is itself an answer, the record MUST stay and report it as such — an explicitly selected "No status" invents nothing, and it is what keeps clearing a status reversible: judged on the folder's word list alone the control would disappear the moment a reader used it. |

...
Decisions 1, 2, 3, 4 and 7 have since landed — the in-process host and its flag,
the `backlog` registration the Dev PC catalog now carries, and the caller-supplied
scope on every tool. With them, the server publishes the delivery engine's
`delivery.surface.lifecycle@1`, so a flow can report a run into the pane instead
of falling through to a dashboard. Each decision below names the code it stands
on; the **Deviations** section says, row by row, which are built and where the
rest still stops short.
...
`AllowedTransitions` ("per `.devbook/domain/tasks/flow.md`"), `IsTransitionAllowed`,
...
- **Hook telemetry has a door beside the tools.** The same listener serves
  ...
  (`.devbook/domain/sessions/domain.md#delivery-run-telemetry`).
...
| 3 — `backlog` server id | **Built.** `.mcp.json` at the repository root declares the project-scope registration, its bearer token supplied by the `headersHelper` `tools/mcp/backlog-mcp-headers.mjs` — `BACKLOG_MCP_TOKEN`, else the app's `settings.json`, re-read by Claude Code after a 401 — rather than a static `${BACKLOG_MCP_TOKEN}` header, which left every session whose environment lacked the variable unauthorized. The Dev PC catalog carries a `backlog` row that `DevToolService` applies as `claude mcp add --transport http --scope user backlog <url> --header "Authorization: Bearer <token>"`. `DevToolMcpMechanism` gained `Http` beside `DotNetTool`, `Command` and `Manual`. The catalog holds `${BACKLOG_MCP_PORT}`/`${BACKLOG_MCP_TOKEN}` placeholders rather than literals — it is committed and resolves out of a synced folder — and the host expands them at apply time, from a closed vocabulary that never reads the environment. |
| 4 — `repository` argument | **Built.** `IRepositoryDirectory.Resolve` does the mapping and `RepositoryScope.Resolve` is what every repository-scoped tool calls before it reads or writes, so an unregistered `owner/name` is a refusal rather than an empty answer. `backlog-run-plan-item` step 2 now reads `repository` off the git remote and carries it on every connector call — the half this row recorded as missing. The delivery surface's eight tools are the documented exception and take a **`worktree`** instead: a run belongs to a checkout rather than to a repository, two worktrees of one clone hold two independent runs, and `start_run` reattaches per skill *within* a worktree. That is the same rule, not a departure from it — the caller still names the scope and the server still infers nothing. |
...
| 7 — feature flags | **Built.** Six groups now, the sixth being `Surface` — the delivery engine's `delivery.surface.lifecycle@1`, eight tools over `IDeliverySurfaceLifecycle`, which had been implemented and tested since the run store landed while nothing published it: a flow resolves a surface by matching those names against its live tool list, so the capability the engine's contract binds ahead of the dashboards resolved to "no surface bound" every time. It shares `SessionFeatures.Sessions` with the `Sessions` group, which is §7 as written rather than an exception to it — the rule is one check per group, each behind its own *context's* flag, and a run recorded into the Sessions pane is that context. `AppFeatures.McpServer` (`"mcp-server"`) exists and `McpServerWorker.ShouldRun` reads it, so the listener holds nothing while the flag is off. `08-crosscutting-concepts.md#feature-enablement` is itself still `proposed` — the current build defaults optional features to enabled and records the disabled ones — so `EnabledByDefault: false` on this key is what keeps the listener off until chosen, the way `SyncFeatures.Sync` does it. The Tools pane still draws the `backlog` row while the flag is off, saying that nothing answers at the registered address: a registration made earlier is still on the machine, and hiding the row would hide the one place that says so. |

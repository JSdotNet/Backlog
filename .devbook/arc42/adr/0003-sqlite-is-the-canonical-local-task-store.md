...
`.devbook/arc42/02-constraints.md` and restated in chapters 04, 06, 07 and 08, all of which
...
- The aggregate is now `TaskItem` in code, and the ubiquitous-language term is
  **Task**. `TaskItem` rather than the bare `Task` because the module's own
  ...
  already means something narrower. The `.devbook/domain` vocabulary has not been renamed

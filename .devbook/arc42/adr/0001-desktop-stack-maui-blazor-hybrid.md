...
in `.devbook/arc42/04-solution-strategy.md` (Technology Choices).
...
(`.devbook/arc42/02-constraints.md`) must still hold:
...
- Local-first constraints (`.devbook/arc42/02-constraints.md`) — canonical markdown on disk,
  native background workers, full offline capability — must not be weakened.
- Reuse across channels is a plus: `.devbook/arc42/04-solution-strategy.md` already selects
...
- This also aligns the Desktop channel with the already-selected Mobile stack
  (.NET MAUI in `.devbook/arc42/04-solution-strategy.md`), enabling future UI/component
...
- `.devbook/arc42/04-solution-strategy.md` — Technology Choices table (Desktop row updated
  to reference this ADR).
- `.devbook/arc42/02-constraints.md` — local-first, markdown-canonical, native
  background-worker constraints this decision preserves.
- `.devbook/arc42/09-architecture-decisions.md` — local system decisions section.

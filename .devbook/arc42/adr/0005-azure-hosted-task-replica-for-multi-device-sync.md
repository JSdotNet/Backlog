...
> (`.devbook/arc42/07-deployment-view.md#provisioning-and-delivery`). The desktop half is
...
inherited ADR 0005 (modular monolith structure) under `.devbook/arc42/adr/guidelines/`.
...
- **Session records.** What the coding agents have been doing, as
  `.devbook/domain/sessions/domain.md#session-log` models it — machine-stamped,
...
`.devbook/arc42/08-crosscutting-concepts.md#storage-and-sync` already commits to it, down
...
`.devbook/arc42/07-deployment-view.md#cloud-deployment-azure` already names Cosmos DB as
...
task database on Azure" breaks all three. `.devbook/arc42/02-constraints.md` requires
...
- **New items always create; edits are last-write-wins on the whole document.**
  This is the policy `.devbook/arc42/08-crosscutting-concepts.md` already states, applied
...
holds the evidence for it. `.devbook/domain/sessions/domain.md#session-log` puts the
...
- **Agent kind, because a session id is not an identity on its own.**
  `.devbook/domain/sessions/domain.md#session-log` puts session identity at the agent plus
...
- **Machine name, because the lookup that was supposed to supply it does not
  ...
  on arrival; `.devbook/domain/sessions/dependencies.md` writes that assumption down as a
...
machines and changed on one — the reasons `.devbook/domain/tasks/domain.md#device` gives
...
- **Resolved repository alias, because Claude records no repository and a scope
  ...
  inference `.devbook/domain/sessions/domain.md#working-location` forbids — and it stays a
...
`.devbook/domain/tasks/domain.md` names `order` and `area` together as the attributes that
...
**Device pairing, not an account.** `.devbook/arc42/02-constraints.md` requires that
...
- Local-first survives fully. Every core workflow still runs offline, and the
  cloud stays additive exactly as `.devbook/arc42/02-constraints.md` requires.
...
- The Task aggregate has gained `updated_at` and `deleted_at`. Both are
  ...
  domain-visible enough to belong in `.devbook/domain/tasks/domain.md`.

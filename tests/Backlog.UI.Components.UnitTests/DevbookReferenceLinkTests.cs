...
///
/// <para>All three shapes used to carry <c>devbook-related</c>, which shares
...
/// </summary>
public sealed class DevbookReferenceLinkTests
{
    private static readonly DevbookReference Chapter =
    ...
    public void A_followable_reference_is_an_anchor_and_carries_nothing_of_the_pill()
    {
        ...
        var link = context.Render<DevbookReferenceLink>(parameters => parameters
            .Add(l => l.Reference, Chapter)
            .Add(l => l.Href, "/knowledge/.tech/shared.md"));

        var anchor = link.Find("a");
        Assert.Equal("devbook-ref devbook-ref--link", anchor.GetAttribute("class"));
        ...
        Assert.DoesNotContain("devbook-related", link.Markup, StringComparison.Ordinal);
        Assert.DoesNotContain("devbook-status", link.Markup, StringComparison.Ordinal);
    }
    ...
    public void The_button_form_is_the_anchor_form_bar_the_modifier()
    {
        ...
        var action = context.Render<DevbookReferenceLink>(parameters => parameters
            .Add(l => l.Reference, Chapter)
            .Add(l => l.OnNavigate, EventCallback.Factory.Create<DevbookReference>(this, _ => { })));

        var button = action.Find("button");
        Assert.Equal("button", button.GetAttribute("type"));
        Assert.Equal("devbook-ref devbook-ref--action", button.GetAttribute("class"));
        Assert.Equal(
            link.Find("a").ClassList.Where(name => name != "devbook-ref--link"),
            button.ClassList.Where(name => name != "devbook-ref--action"));
    ...
    public void A_reference_nobody_can_follow_is_inert_text_and_is_not_link_coloured()
    {
        ...
        var inert = context.Render<DevbookReferenceLink>(parameters => parameters
            .Add(l => l.Reference, Chapter));

        var code = inert.Find("code");
        Assert.Equal("devbook-ref devbook-ref--inert", code.GetAttribute("class"));
        ...
        Assert.DoesNotContain("devbook-ref--link", code.ClassList);
        Assert.DoesNotContain("devbook-ref--action", code.ClassList);
    ...
    public void Activating_a_button_reports_the_whole_reference()
    {
        ...
        var link = context.Render<DevbookReferenceLink>(parameters => parameters
            .Add(l => l.Reference, Chapter)
            .Add(l => l.OnNavigate, EventCallback.Factory.Create<DevbookReference>(this, followed.Add)));

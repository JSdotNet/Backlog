...
public sealed class DomainDevbookStoreTests : IDisposable
{
    ...
    {
        var view = await new DomainDevbookStore(new DevbookFolderSource(NewSettingsStore())).LoadAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Reads_context_map_contexts_metadata_links_and_diagrams()
    {
        ...
        var view = await new DomainDevbookStore(new DevbookFolderSource(settings)).LoadAsync(cancellationToken: TestContext.Current.CancellationToken);
        ...
        var model = context.Documents.Single(d => d.Kind == DomainDevbookDocumentKind.Model);
        ...
        Assert.Equal("planned", DevbookActionMetadata.Status(features.Metadata));
        var issue = DevbookActionMetadata.Issue(features.Metadata, view.RepositoryLabel);
    ...
    public async Task Honors_repository_relative_domain_folder_override()
    {
        ...
        settings.SetRepositories(repositories);
        settings.SetCloneDirectory("backlog", repo);
        settings.SetDevbookFolder("backlog", ".domain", enabled: true, path: "knowledge/.domain");
    ...
    public async Task Updates_document_status_metadata()
    {
        ...
        WriteDomain(repo);
        var settings = ConfiguredSettings(repo);
        var store = new DomainDevbookStore(new DevbookFolderSource(settings));
        ...
        var features = Assert.Single(Assert.Single(view.Contexts).Documents, d => d.Kind == DomainDevbookDocumentKind.Features);
    ...
    public async Task Updates_section_status_metadata()
    {
        ...
        var section = Assert.Single(Assert.Single(view.Contexts).Documents.Single(d => d.Kind == DomainDevbookDocumentKind.Features).Sections);
    ...
    public async Task Defers_reading_a_context_until_its_documents_are_asked_for()
    {
        ...
        var context = Assert.Single(view.Contexts);
        var documents = Assert.IsType<LazyDevbookList<DomainDevbookDocument>>(context.Documents);
        Assert.False(documents.IsMaterialized);

        var domain = context.Documents.Single(document => document.Kind == DomainDevbookDocumentKind.Domain);
    ...
    public async Task Falls_back_to_scanning_the_folder_when_no_index_is_present()
    {
        ...
        Assert.Equal("Inbox", context.DisplayName);
        Assert.Equal("active", context.Status);
        Assert.Contains(context.Documents, document => document.Kind == DomainDevbookDocumentKind.Domain);
    }
    ...
    public async Task Falls_back_to_scanning_when_the_index_declares_an_unknown_schema_version()
    {
        ...
        Assert.IsNotType<LazyDevbookList<DomainDevbookDocument>>(context.Documents);
    }
...
""");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "domain-devbook-tests", Guid.NewGuid().ToString("n"));
        _tempDirs.Add(path);
        return path;
    }

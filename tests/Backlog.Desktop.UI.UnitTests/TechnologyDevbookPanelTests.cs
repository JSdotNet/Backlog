...
/// </para>
/// </summary>
public sealed class TechnologyDevbookPanelTests : IDisposable
{
    ...
    public async Task The_pane_opens_on_the_graph_with_the_layer_detail_behind_the_second_tab()
    {
        ...
        Assert.Empty(component.FindAll(".devbook-pane__source"));
        Assert.Empty(component.FindAll(".devbook-pane__subtitle"));
        Assert.Single(component.FindAll(".devbook-pane__header [data-testid='technology-open-vscode-button']"));
    ...
    public async Task A_status_change_reaches_the_layer_file()
    {
        await using var harness = CreateHarness();
        var component = harness.RenderLayers();
        component.WaitForAssertion(() => Assert.NotEmpty(component.FindAll(".tech-layer__header [data-testid='devbook-state-select'] select")));

        component.Find(".tech-layer__header [data-testid='devbook-state-select'] select").Change("adopted");
    ...
    private Harness CreateHarness(bool withTechFolder = true)
    {
        ...
        var folders = new RecordingDevbookFolderSource(
            new DevbookFolderSource(gitHubSettings),
    ...
    public async Task The_sheet_offers_the_folders_own_ladder_and_writes_it_back()
    {
        ...
        Assert.Equal(DevbookStatus.Values(DevbookFolder.Tech), offered.Take(5));
    ...
    private sealed record Harness(BunitContext Context, string TechFolder, RecordingDevbookFolderSource Folders) : IAsyncDisposable
    {
        ...
        public IRenderedComponent<TechnologyDevbookPanel> RenderLayers()
        {

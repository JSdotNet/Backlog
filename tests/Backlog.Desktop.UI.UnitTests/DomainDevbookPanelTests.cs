...
/// </para>
/// </summary>
public sealed class DomainDevbookPanelTests : IDisposable
{
    ...
    public async Task Edit_puts_the_editing_surface_in_the_file_views_own_body()
    {
        ...
        component.WaitForAssertion(() => Assert.Single(component.FindAll("[data-testid='domain-chapter-file-body'] [data-testid='devbook-chapter-surface']")));
    ...
    public async Task The_shown_chapter_is_the_file_and_nothing_after_it()
    {
        ...
        // not in the file.
        Assert.Single(component.FindAll("[data-testid='devbook-copilot-cli-button']"));
    }
    ...
    public async Task A_status_the_body_shows_is_not_offered_a_second_time_beside_it()
    {
        ...
        Assert.Single(component.FindAll(".file-view__header .devbook-record__headline"));
        Assert.Empty(component.FindAll("[data-testid='domain-chapter-file-body'] .devbook-record"));
    ...
    public async Task The_read_view_shows_the_status_and_not_the_rest_of_the_fence()
    {
        ...
        Assert.Single(component.FindAll(".devbook-record__headline .badge--status"));
        Assert.Empty(component.FindAll("dl.devbook-fields"));
    }
    ...
    {
        var followed = new List<DevbookChapterLink>();
        await using var harness = CreateHarness();
    ...
    private static DomainDevbookView ContextsView()
    {
        var absent = Path.Combine(Path.GetTempPath(), "backlog-domain-panel-contexts", Guid.NewGuid().ToString("N"));

        static DomainDevbookDocument Document(string path, string title, DomainDevbookDocumentKind kind, string status) =>
            new(path,
                title,
                kind,
                status,
        ...
        return new DomainDevbookView(
            "JSdotNet/Backlog",
            absent,
            Path.Combine(absent, ".domain"),
            null,
            Document(ContextMapPath, "Context Map", DomainDevbookDocumentKind.ContextMap, "draft"),
            [
                new DomainDevbookContext("capture", "Capture", "proposed",
                    [Document(".domain/capture/domain.md", "Domain: Capture", DomainDevbookDocumentKind.Domain, "proposed")]),
                new DomainDevbookContext("tasks", "Tasks", "draft",
                    [Document(".domain/tasks/domain.md", "Domain: Tasks", DomainDevbookDocumentKind.Domain, "draft")])
            ]);
    }

    private static DomainDevbookView MissingRootView()
    {
        ...
        return new DomainDevbookView(
            ...
            Path.Combine(absent, ".domain"),
            null,
            new DomainDevbookDocument(
                ContextMapPath,
                "Context Map",
                DomainDevbookDocumentKind.ContextMap,
                "draft",
                new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase),
                "A summary nobody can edit.",
                [],
    ...
    private static DomainDevbookView RelatedView()
    {
    ...
    private static DomainDevbookView CrossAreaView()
    {
    ...
    /// state - <see cref="DevbookIssueLink"/> carries a repo, a number and a
    ...
    public async Task A_linked_issue_is_not_drawn_as_an_open_one()
    {
        ...
        component.WaitForAssertion(() => Assert.NotEmpty(component.FindAll("[data-testid='devbook-issue-badge']")));

        Assert.All(
            component.FindAll("[data-testid='devbook-issue-badge']"),
            badge =>
            {
    ...
    private static DomainDevbookView IssueLinkedView()
    {
    ...
    private Harness CreateHarness(StubGitFileHistory? history = null, bool diagramChapter = false, IDevbookAnnotationStore? annotations = null)
    {
        ...
        var folders = new RecordingDevbookFolderSource(
            new DevbookFolderSource(gitHub, settings),
    ...
    {
        public IRenderedComponent<DomainDevbookPanel> Render(string? selectedPath) =>
            Context.Render<DomainDevbookPanel>(parameters => parameters
        ...
        public IRenderedComponent<DomainDevbookPanel> RenderView(
            DomainDevbookView view,
            string? selectedPath,
            Action<DevbookChapterLink>? onNavigate = null) =>

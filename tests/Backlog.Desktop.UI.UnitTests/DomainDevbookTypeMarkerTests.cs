...
/// </summary>
public sealed class DomainDevbookTypeMarkerTests : IDisposable
{
    ...
    public void A_chapter_heading_leads_with_its_type_and_still_reads_as_itself()
    {
        ...
        Assert.Contains("devbook-type-marker--aggregate", mark.ClassList);
    ...
    public void The_document_header_marks_what_kind_of_file_it_is()
    {
        ...
        Assert.Contains("devbook-type-marker--model", mark.ClassList);
    ...
    public async Task An_opened_chapter_whose_type_nobody_drew_is_left_exactly_as_it_was()
    {
        ...
        // survived. This panel passes RenderDevbookMetadataFields="false", so no
        ...
        // DevbookTypeMarkerReadSurfaceTests is where it is pinned.
    }
    ...
    {
        DevbookMenuNode File(string path, string label) =>
            new(path, label, path, DevbookMenuNodeKind.File, "domain", [], true);

        var inbox = new DevbookMenuNode("inbox", "Inbox", "inbox", DevbookMenuNodeKind.Folder, "domain",
        [
        ...
        return new DevbookMenuNode("domain", "Domain", "domain", DevbookMenuNodeKind.Folder, "domain",
    ...
    /// them.</para>
    /// </summary>
    private static DomainDevbookView ChapterView(string chapterType, string? related = ".domain/capture/model.md#capture")
    {
        ...
        var chapter = new DomainDevbookSection(
            "Inbox Item",
            2,
            "draft",
            chapterMetadata,
        ...
        var document = new DomainDevbookDocument(
            DocumentPath,
            "Inbox",
        ...
        return new DomainDevbookView("JSdotNet/Backlog", absent, Path.Combine(absent, ".domain"), null, document, []);
    }
    ...
    private sealed record Harness(BunitContext Context) : IDisposable
    {
        ...
        public IRenderedComponent<DomainDevbookPanel> Render(DomainDevbookView view) =>
            Context.Render<DomainDevbookPanel>(parameters => parameters.Add(panel => panel.View, view));
    ...
    private sealed record OpenHarness(BunitContext Context, string RepositoryAlias) : IAsyncDisposable
    {
        ...
        {
            var panel = Context.Render<DomainDevbookPanel>(parameters => parameters
        ...
        /// <summary>Awaited, for the reason <c>DomainDevbookPanelTests</c> records:

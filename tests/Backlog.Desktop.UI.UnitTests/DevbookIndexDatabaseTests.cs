...
public sealed class DevbookIndexDatabaseTests : IDisposable
{
    ...
    {
        var folder = Arrange(schemaVersion: DevbookDatabaseSchema.Version + 1);
    ...
    {
        Assert.Null(DevbookIndexDocument.TryRead(Arrange(withDatabase: false)));
    }
    ...
    private string Arrange(bool withDatabase = true, int? schemaVersion = null)
    {
        ...
        Execute(
            connection,
            $"INSERT INTO meta (key, value) VALUES ('schemaVersion', '{schemaVersion ?? DevbookDatabaseSchema.Version}')");

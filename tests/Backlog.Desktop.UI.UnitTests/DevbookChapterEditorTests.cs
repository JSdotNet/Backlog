...
public sealed class DevbookChapterEditorTests : IDisposable
{
    ...
    public async Task A_chapter_that_could_not_be_resolved_offers_no_way_in()
    {
        ...
        var component = context.Render<DevbookChapterEditor>(parameters => parameters
    ...
    public async Task A_resolved_chapter_offers_a_way_in()
    {
        ...
        Assert.Single(component.FindAll("[data-testid='devbook-chapter-edit']"));
    }
    ...
    public async Task Done_flushes_the_pending_body_and_returns_to_reading()
    {
        ...
        component.Find("[data-testid='devbook-chapter-edit']").Click();
        component.Find("textarea").Input("# Notes\n\nTyped before Done.\n");
        component.Find("[data-testid='devbook-chapter-done']").Click();
    ...
    public async Task Moving_to_another_chapter_flushes_the_one_being_left()
    {
        ...
        var second = new DevbookChapterRef("arc42", root, "second.md");
    ...
    public async Task A_write_that_fails_says_so_rather_than_looking_saved()
    {
        ...
        component.WaitForAssertion(
            () => Assert.Contains("Could not save", component.Find("[data-testid='devbook-chapter-save-state']").TextContent, StringComparison.Ordinal),
            TimeSpan.FromSeconds(5));
        Assert.NotEmpty(component.FindAll("[data-testid='devbook-chapter-save-error']"));
    }
    ...
    public async Task A_saved_chapter_says_so()
    {
        ...
        component.WaitForAssertion(
            () => Assert.Equal("Saved", component.Find("[data-testid='devbook-chapter-save-state']").TextContent.Trim()),
            TimeSpan.FromSeconds(5));
    }
    ...
    public async Task A_debounced_save_that_fails_unexpectedly_says_so_rather_than_saving_forever()
    {
        ...
        Assert.Contains(
            ...
            component.Find("[data-testid='devbook-chapter-save-error']").TextContent,
    ...
    public async Task A_surface_asked_to_fill_stops_being_sized_by_its_own_content()
    {
        ...
        var surface = component.Find("[data-testid='devbook-chapter-surface']");

        Assert.Contains("devbook-chapter-editor--fill", surface.ClassList);
    ...
    public async Task A_surface_nobody_sized_renders_exactly_as_it_did()
    {
        ...
        Assert.DoesNotContain(
            "devbook-chapter-editor--fill",
            component.Find("[data-testid='devbook-chapter-surface']").ClassList);
    ...
    private static IRenderedComponent<DevbookChapterEditor> Render(BunitContext context, DevbookChapterRef chapter, string text) =>
        context.Render<DevbookChapterEditor>(parameters => parameters
            .Add(editor => editor.Chapter, chapter)
    ...
    private (string Root, DevbookChapterRef Chapter) Chapter(string relativePath, string markdown)
    {
        var root = Path.Combine(Path.GetTempPath(), "devbook-chapter-editor-tests", Guid.NewGuid().ToString("N"));

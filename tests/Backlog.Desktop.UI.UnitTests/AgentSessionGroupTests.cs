...
public sealed class AgentSessionGroupTests
{
    ...
    {
        Assert.Equal(["Claude", "Copilot"], AgentSessionGroups.Of(Rows(Sample), AgentSessionGrouping.Kind).Select(group => group.Key));
        Assert.Null(Assert.Single(AgentSessionGroups.Of(Rows(Sample), AgentSessionGrouping.None)).Key);
    }

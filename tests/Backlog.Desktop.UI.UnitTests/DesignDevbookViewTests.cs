...
/// </para>
/// </summary>
public sealed class DesignDevbookViewTests : IDisposable
{
    ...
    public async Task Edit_puts_the_editing_surface_in_the_file_views_own_body()
    {
        ...
        component.WaitForAssertion(() => Assert.Single(component.FindAll("[data-testid='design-chapter-file-body'] [data-testid='devbook-chapter-surface']")));
    ...
    public async Task The_selected_chapter_is_offered_whole_rather_than_reassembled_from_its_sections()
    {
        ...
        component.WaitForAssertion(() => Assert.Contains(
            "status: accepted",
            component.Find("[data-testid='devbook-chapter-surface']").TextContent,
    ...
    public async Task Every_chapter_of_the_open_file_carries_its_own_record_and_no_raw_fence()
    {
        ...
        Assert.Equal(
            ...
            body.QuerySelectorAll(".devbook-record__headline .md-heading--2").Select(heading => heading.TextContent.Trim()));
        Assert.Equal(2, body.QuerySelectorAll(".devbook-record__headline .badge--status").Length);
    ...
    public async Task The_open_files_own_block_is_drawn_in_the_header_and_nowhere_else()
    {
        ...
        Assert.DoesNotContain(
            body.QuerySelectorAll(".devbook-record"),
        ...
        Assert.Empty(body.QuerySelectorAll(".devbook-record .md-heading--1"));
    }
    ...
    public async Task A_status_picked_on_a_chapter_of_the_open_file_is_written_to_that_chapters_fence()
    {
        await using var harness = CreateHarness();
        var component = harness.Render("interaction-guidelines.md");
        component.WaitForAssertion(() => Assert.NotEmpty(component.FindAll("[data-testid='design-chapter-file-body'] .devbook-record select")));

        component.FindAll("[data-testid='design-chapter-file-body'] .devbook-record select")[0].Change("deprecated");
    ...
    public async Task A_design_block_is_drawn_as_the_record_it_is_and_not_as_the_fence_it_was_written_in()
    {
        ...
        Assert.Empty(component.FindAll("span.devbook-status"));
        Assert.Empty(component.FindAll("code.devbook-related"));
    }
    ...
    public async Task Each_design_subject_shows_its_status_once()
    {
        ...
        Assert.Equal(2, colors.QuerySelectorAll(".devbook-record").Length);
        ...
        Assert.Equal(2, colors.QuerySelectorAll(".devbook-record__headline .badge--status").Length);
    }
    ...
    {
        public IRenderedComponent<DesignDevbookView> Render(string? selectedPath) =>
        ...
        public void Rerender(IRenderedComponent<DesignDevbookView> component, string? selectedPath) =>

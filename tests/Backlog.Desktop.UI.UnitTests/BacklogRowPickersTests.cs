...
/// </para>
/// </summary>
public class BacklogRowPickersTests
{
    private static string RowTestId(EntryRow row) => $"entry-list-{(row.Id ?? row.Key)}";

    private static IElement Row(IRenderedComponent<BacklogPane> pane, EntryRow row) =>
        pane.Find($"[data-testid='{RowTestId(row)}']");

    private static IElement Picker(IRenderedComponent<BacklogPane> pane, EntryRow row, string testId) =>

...
/// </summary>
public sealed class TasksRefreshSettingsStoreTests
{
    [Fact]
    public void Polling_is_on_at_a_sensible_interval_before_anybody_chooses()
    {
        ...
        try
        {
            var store = new TasksRefreshSettingsStore(path);

            Assert.True(store.Current.PollingEnabled);
            Assert.Equal(TasksRefreshSettings.DefaultPollingIntervalSeconds, store.Current.PollingIntervalSeconds);
    ...
    public void Both_choices_survive_a_restart()
    {
        ...
        try
        {
            ...
            var restarted = new TasksRefreshSettingsStore(path);
    ...
    public void An_interval_below_a_second_is_refused_rather_than_saved(int seconds)
    {
        ...
        try
        {
            ...
            Assert.Equal(
                ...
                new TasksRefreshSettingsStore(path).Current.PollingIntervalSeconds);
        }
        finally
        {
            DeleteSettingsDirectory(path);
    ...
    public void The_shortest_allowed_interval_is_accepted()
    {
        ...
        try
        {
            ...
            Assert.Null(store.SetPollingIntervalSeconds(TasksRefreshSettings.MinimumPollingIntervalSeconds));
    ...
    public void A_corrupt_or_partial_file_falls_back_to_the_defaults()
    {
        ...
        try
        {
            ...
            Assert.True(new TasksRefreshSettingsStore(path).Current.PollingEnabled);

            File.WriteAllText(path, JsonSerializer.Serialize(new { pollingEnabled = false }));

            var partial = new TasksRefreshSettingsStore(path);

            Assert.False(partial.Current.PollingEnabled);
            Assert.Equal(TasksRefreshSettings.DefaultPollingIntervalSeconds, partial.Current.PollingIntervalSeconds);
        }
        finally
        {
            DeleteSettingsDirectory(path);

...
/// <summary>
/// <see cref="IDevbookFolderSource"/> publishes <c>Changed</c> so that open
...
public sealed class DevbookFolderChangeRefreshTests
{
    ...
    {
        using var workspace = DevbookWorkspace.Create();
        var provider = new DesignDevbookProvider(workspace.Folders);
        var raised = 0;
        void Handler() => raised++;
    ...
    [Fact]
    public void Devbook_menu_publishes_the_folder_source_change()
    {
        using var workspace = DevbookWorkspace.Create();
        var menu = new DevbookMenu(workspace.Folders);
        var raised = 0;
        void Handler() => raised++;
    ...
    [Fact]
    public void Devbook_menu_stops_publishing_once_unsubscribed()
    {
        using var workspace = DevbookWorkspace.Create();
        var menu = new DevbookMenu(workspace.Folders);
        var raised = 0;
    ...
    /// contract <see cref="Arc42DevbookPanel"/> states for its own handler,
    ...
    /// reach <see cref="DevbookChapterEditor"/> differently — a move changes the
    ...
    [Fact]
    public void Devbook_pane_menu_reloads_when_the_knowledge_folder_moves()
    {
        using var workspace = DevbookWorkspace.Create();
        ...
        var component = context.Render<DevbookPane>(parameters => parameters
            .Add(parameter => parameter.RepositoryAlias, DevbookWorkspace.Alias));
    ...
    /// </summary>
    [Fact]
    public void Devbook_pane_sections_follow_a_knowledge_folder_being_turned_off()
    {
        using var workspace = DevbookWorkspace.Create();
        workspace.SetFolderEnabled("instructions", false);
        workspace.SetFolderEnabled(".arc42", false);
    ...
    public void Devbook_pane_attaches_one_handler_to_the_folder_source()
    {
        ...
        context.Render<DevbookPane>(parameters => parameters
            .Add(parameter => parameter.RepositoryAlias, DevbookWorkspace.Alias));
    ...
    [Fact]
    public async Task Devbook_pane_detaches_from_the_folder_source_when_disposed()
    {
        using var workspace = DevbookWorkspace.Create();
        workspace.SetFolderEnabled("instructions", false);
        workspace.SetFolderEnabled(".arc42", false);
    ...
    private static string MenuMarkup(IRenderedComponent<DevbookPane> component) =>
        component.Find(".devbook-stack__menu").InnerHtml;
    ...
    /// instructions panel's subtree contains <see cref="DevbookChapterEditor"/>,
    ...
    /// </summary>
    [Fact]
    public async Task Devbook_panels_detach_from_the_folder_source_when_disposed()
    {
        using var workspace = DevbookWorkspace.Create();
        var source = new CountingDevbookFolderSource(workspace.Folders);
        var context = workspace.CreateBunitContext(source);
        ...
        context.Render<DomainDevbookPanel>(parameters => parameters
            .Add(parameter => parameter.RepositoryAlias, DevbookWorkspace.Alias));
        context.Render<DesignDevbookView>(parameters => parameters
            .Add(parameter => parameter.RepositoryAlias, DevbookWorkspace.Alias));
    ...
    public void Domain_knowledge_panel_keeps_a_parent_supplied_view_across_a_folder_move()
    {
        using var workspace = DevbookWorkspace.Create();
        using var context = workspace.CreateBunitContext();
        var supplied = DomainDevbookView.Unavailable("Supplied by the parent.");
...
file sealed class DevbookWorkspace : IDisposable
{
    ...
    private DevbookWorkspace(string root)
    {
        _root = root;
    ...
    public DevbookFolderSource Folders { get; }

    public static DevbookWorkspace Create() =>
        new(Path.Combine(Path.GetTempPath(), "backlog-devbook-refresh-tests", Guid.NewGuid().ToString("n")));
    ...
    public BunitContext CreateBunitContext(IDevbookFolderSource? folders = null)
    {
...
/// </summary>
file sealed class CountingDevbookFolderSource(IDevbookFolderSource inner) : IDevbookFolderSource
{

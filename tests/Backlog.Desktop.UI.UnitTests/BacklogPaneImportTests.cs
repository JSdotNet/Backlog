...
/// (<c>Backlog.Modules.Backlog.UnitTests.ImportPlanTests</c>). What is under test
/// here is only the routing <c>BacklogPane.OnImportAsync</c> performs — a result
...
public sealed class BacklogPaneImportTests
{
    ...
    private static async Task SubmitAsync(IRenderedComponent<BacklogPane> pane, string plan)
    {
        pane.Find("[data-testid='import-plan-open']").Click();

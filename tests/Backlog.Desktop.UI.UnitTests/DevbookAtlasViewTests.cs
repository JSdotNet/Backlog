...
/// </summary>
public sealed class DevbookAtlasViewTests : IDisposable
{
    ...
    public void Selecting_a_document_opens_the_sheet_on_it()
    {
        ...
        Assert.Equal("false", component.Find("[data-testid=\"devbook-atlas-sheet\"]").GetAttribute("data-open"));

        component.FindAll("[data-testid=\"graph-atlas-index-option\"]")[0].Click();

        var sheet = component.Find("[data-testid=\"devbook-atlas-sheet\"]");
    ...
    {
        public IRenderedComponent<DevbookAtlasView> Render(DevbookAtlasScope scope) =>
            Context.Render<DevbookAtlasView>(parameters =>
            {

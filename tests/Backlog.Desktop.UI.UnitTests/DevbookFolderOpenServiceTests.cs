...
public sealed class DevbookFolderOpenServiceTests : IDisposable
{
    ...
    public async Task Opens_nested_knowledge_folder_in_configured_area()
    {
        ...
        var service = new DevbookFolderOpenService(new DevbookFolderSource(NewSettingsStore(repo)), launcher);
    ...
    public async Task Rejects_paths_outside_knowledge_root()
    {
        ...
        var service = new DevbookFolderOpenService(new DevbookFolderSource(NewSettingsStore(repo)), new RecordingFolderEditorLauncher());

        await Assert.ThrowsAsync<DevbookFolderOpenException>(() => service.OpenAsync("domain", "../outside", cancellationToken: TestContext.Current.CancellationToken));
    }
    ...
    public async Task Reports_a_launcher_failure_as_a_knowledge_failure_without_rewording_it()
    {
        ...
        var service = new DevbookFolderOpenService(
            new DevbookFolderSource(NewSettingsStore(repo)),
            new ThrowingFolderEditorLauncher(failure));

        var error = await Assert.ThrowsAsync<DevbookFolderOpenException>(() => service.OpenAsync("domain", ".domain", cancellationToken: TestContext.Current.CancellationToken));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "devbook-folder-open-tests", Guid.NewGuid().ToString("n"));
        _tempDirs.Add(path);
        return path;
    }

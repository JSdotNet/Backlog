...
public sealed class BacklogCopilotCliTests : IDisposable
{
    ...
    public async Task An_entry_launches_copilot_with_its_markdown_as_the_prompt()
    {
        ...
        var integration = new BacklogCopilotCli(launcher);
    ...
    public async Task Starting_copilot_records_usage_on_the_saved_entry()
    {
        ...
        var reloaded = await BacklogTestHost.RepositoryFor(harness.Store).GetAsync(row.Id!.Value);
        Assert.Equal(BacklogCopilotCli.UsageAction, Assert.Single(reloaded!.UsageEvents).Action);
    }

...
/// </para>
/// </summary>
public sealed class DevbookReadOnlyBranchTests : IDisposable
{
    ...
    {
        var view = await new DomainDevbookStore(BranchSource(WithDomain()))
    ...
    {
        var view = await new DomainDevbookStore(LocalSource(WithDomain()))
            .LoadAsync("backlog", TestContext.Current.CancellationToken);
    ...
    {
        var store = new DomainDevbookStore(BranchSource(WithDomain()));
    ...
    public async Task A_refused_status_change_leaves_the_file_alone()
    {
        ...
        var store = new DomainDevbookStore(BranchSource(root));
    ...
    public void A_chapter_under_a_branch_resolves_but_is_not_editable()
    {
        ...
        var chapter = DevbookChapterResolver.TryResolve("domain", location, ".domain/inbox/domain.md");
    ...
    {
        var root = WithDomain();
        var chapter = new DevbookChapterRef(
            "domain",
        ...
        var failure = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            new DevbookChapterWriter().WriteAsync(chapter, "# Replaced", cancellationToken: TestContext.Current.CancellationToken));
    ...
    {
        var root = WithDomain();
        var chapter = new DevbookChapterRef("domain", Path.Combine(root, ".domain"), "inbox/domain.md");

        await new DevbookChapterWriter().WriteAsync(chapter, "# Replaced\n", cancellationToken: TestContext.Current.CancellationToken);
    ...
    private IDevbookFolderSource BranchSource(string root) => new StubFolderSource(root, DevbookSourceKind.Branch);

    private IDevbookFolderSource LocalSource(string root) => new StubFolderSource(root, DevbookSourceKind.LocalFolder);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "devbook-read-only-tests-" + Guid.NewGuid().ToString("N"));
    ...
    /// <para>
    /// Deliberately not the real <c>DevbookFolderSource</c> over a real

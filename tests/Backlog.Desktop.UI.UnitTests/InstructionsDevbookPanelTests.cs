...
public sealed class InstructionsDevbookPanelTests
{
    [Fact]
    ...
    public async Task The_selected_document_is_shown_through_the_shared_file_view()
    {
        ...
        component.AssertTheFileIsNamedOnce("copilot-instructions.md", "[data-testid='instructions-devbook-panel']");
    }
    ...
    public async Task Edit_puts_the_editing_surface_in_the_file_views_own_body()
    {
        ...
        component.WaitForAssertion(() => Assert.Single(component.FindAll("[data-testid='instructions-document-body'] [data-testid='devbook-chapter-surface']")));
    ...
    private static async Task ClickTreeRowAsync(IRenderedComponent<InstructionsDevbookPanel> component, string label)
    {
        component.WaitForAssertion(() => Assert.Contains(
            component.FindAll("[data-testid='instructions-reach-view'] [role='treeitem']"),

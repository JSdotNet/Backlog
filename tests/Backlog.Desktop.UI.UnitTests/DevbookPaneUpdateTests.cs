...
/// </para>
/// </summary>
public sealed class DevbookPaneUpdateTests
{
    ...
    [Fact]
    public async Task Devbook_read_from_a_clone_offers_to_check_the_version_before_anybody_has_asked()
    {
        ...
        Assert.Empty(component.FindAll("[data-testid='devbook-update-behind']"));
    ...
    public async Task A_clone_on_the_latest_version_says_so_and_keeps_offering_the_check()
    {
        ...
        {
            var status = component.Find("[data-testid='devbook-update-status']");
            Assert.Equal("On the latest version of origin/main.", status.TextContent.Trim());
            Assert.Contains("devbook-stack__update-status--ok", status.GetAttribute("class"));
            Assert.Equal("Check now", ActionButton(component).TextContent.Trim());
        });
    ...
    public async Task A_clone_the_remote_has_moved_past_shows_how_far_behind_and_turns_the_button_into_a_pull()
    {
        ...
        component.WaitForAssertion(() =>
        {
            Assert.Equal("3 behind", component.Find("[data-testid='devbook-update-behind']").TextContent.Trim());
            ...
            Assert.Contains("devbook-stack__update-status--available", status.GetAttribute("class"));
        });
    }
    ...
    public async Task A_refused_pull_says_why_and_leaves_the_button_offering_a_check()
    {
        ...
        component.WaitForAssertion(() =>
        {
            ...
            Assert.Contains("devbook-stack__update-status--blocked", status.GetAttribute("class"));
            Assert.Equal("Check now", ActionButton(component).TextContent.Trim());
        });
    ...
    public async Task The_version_control_renders_the_librarys_button_rather_than_a_bare_one()
    {
        ...
        Assert.Equal("devbook-stack__update-toggle", button.GetAttribute("class"));
    }
    ...
    private static AngleSharp.Dom.IElement ActionButton(IRenderedComponent<DevbookPane> component) =>
        component.Find("[data-testid='devbook-update-action']");

    private static string MenuText(IRenderedComponent<DevbookPane> component) =>
        string.Join(" ", component.FindAll(".devbook-menu__item").Select(item => item.TextContent));

    private static Harness CreateHarness(bool cloned = true)
    {
        var root = Path.Combine(Path.GetTempPath(), "backlog-devbook-pane-update-tests", Guid.NewGuid().ToString("n"));
        var clone = Path.Combine(root, "clone");
        WriteDevbookFolders(clone);
        ...
        ScriptedGitRepositoryService Git) : IAsyncDisposable
    {
        ...
        public IRenderedComponent<DevbookPane> RenderWithoutRepository() =>
            Context.Render<DevbookPane>(parameters => parameters.Add(pane => pane.RepositoryAlias, null));
        ...
        /// <summary>Awaited for the reason <c>DevbookPaneOpenErrorTests</c>

...
public sealed class DevbookActionMetadataTests
{
    [Theory]
    ...
    {
        var issue = DevbookActionMetadata.Issue(new Dictionary<string, string> { ["devbook-issue"] = value }, repository);
    ...
    public void Issue_ignores_empty_and_null_values(string? value)
    {
        var metadata = value is null
            ...
            : new Dictionary<string, string> { ["devbook-issue"] = value };

        Assert.Null(DevbookActionMetadata.Issue(metadata, "JSdotNet/Backlog"));
    }

    [Fact]
    public void BuildPrompt_describes_knowledge_item_context()
    {
        var prompt = DevbookActionMetadata.BuildPrompt(new DevbookActionItem(
            "Inbox capture",
            "Feature",
        ...
        Assert.Contains("Devbook item: Inbox capture", prompt);

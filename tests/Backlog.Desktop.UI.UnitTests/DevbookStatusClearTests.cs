...
/// </summary>
public class DevbookStatusClearTests
{
    [Fact]
    public void Keeps_the_fence_when_the_status_was_the_only_field_in_it()
    {
        ...
        DevbookMarkdownStatusWriter.RemoveStatus(file.FolderRoot, ".design/color-scheme.md", ".design/");
    ...
    public void Clearing_one_chapter_leaves_its_siblings_fence_alone()
    {
        ...
        DevbookMarkdownStatusWriter.RemoveStatus(file.FolderRoot, ".design/color-scheme.md#surfaces", ".design/");
    ...
    public void A_cleared_chapter_reads_back_as_an_addressable_chapter_with_an_empty_record()
    {
        ...
        var document = DevbookMarkdownParser.Parse(".design/color-scheme.md", file.Read());

        Assert.Equal("Colour Scheme", document.Title);
        Assert.Contains(document.Blocks, block => block is DevbookHeadingBlock { Level: 2, Text: "Surfaces" });
    }
    ...
    public async Task An_arc42_chapter_can_have_its_status_removed_through_the_store()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(root, ".arc42", "04-solution-strategy.md"), """
                ...
                Strategy text.
                """);
            var knowledge = new Arc42DevbookStore(new DevbookFolderSource(ConfiguredSettings(root)));
    ...
    public async Task A_domain_chapter_can_have_its_status_removed_through_the_store()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(root, ".domain", "tasks", "features.md"), """
                ...
                Capture text.
                """);
            var knowledge = new DomainDevbookStore(new DevbookFolderSource(ConfiguredSettings(root)));
    ...
    {
        Assert.Null(typeof(TechnologyDevbookService).GetMethod("ClearStatusAsync"));
        Assert.NotNull(typeof(Arc42DevbookStore).GetMethod("ClearStatusAsync"));
        Assert.NotNull(typeof(DomainDevbookStore).GetMethod("ClearStatusAsync"));
        Assert.NotNull(typeof(DesignDevbookProvider).GetMethod("ClearStatusAsync"));
    }
    ...
    private static string NewRoot() =>
        Path.Combine(Path.GetTempPath(), "backlog-devbook-clear-tests", Guid.NewGuid().ToString("N"));

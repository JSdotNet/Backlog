...
public class Arc42DevbookTests
{
    ...
    {
        var document = DevbookMarkdownParser.Parse(".arc42/05-building-block-view.md", "# Building Block View\n\n## Level 1\n\nDetails");

        Assert.Equal("Building Block View", document.Title);
        Assert.Contains(document.Blocks, block => block is DevbookHeadingBlock { Level: 2, Text: "Level 1" });
    ...
    public void Content_blocks_skip_document_heading_metadata()
    {
        ...
        Assert.Contains(document.Blocks, block => block is DevbookHeadingBlock { Level: 1, Text: "ADR 0001: Test decision" });
        Assert.DoesNotContain(document.ContentBlocks, block => block is DevbookHeadingBlock { Level: 1 });
        var statusHeading = Assert.IsType<DevbookHeadingBlock>(document.ContentBlocks.First());
    ...
    public void Turns_mermaid_fences_into_reusable_diagram_blocks()
    {
        ...
        var diagram = Assert.IsType<DevbookDiagramBlock>(document.Blocks.Single(block => block is DevbookDiagramBlock));
    ...
    public async Task Reads_arc42_index_order_when_available()
    {
        ...
        try
        {
            ...
            var catalog = await Arc42DevbookReader.LoadAsync(root);
    ...
    public async Task Reads_a_remapped_folders_index_against_the_repository_root()
    {
        ...
        try
        {
            ...
            var catalog = await Arc42DevbookReader.LoadFolderAsync(folder, root);
    ...
    public async Task Counts_only_adr_and_tdr_files_as_decision_records()
    {
        ...
        try
        {
            ...
            Assert.False(Arc42DevbookCatalog.IsDecisionRecord(".arc42/01-introduction-and-goals.md"));
            Assert.True(Arc42DevbookCatalog.IsDecisionRecord(".arc42/adr/0001-test.md"));
            Assert.True(Arc42DevbookCatalog.IsDecisionRecord(".arc42/tdr/0001-test.md"));
        }
        finally
        {
            if (Directory.Exists(root)) Directory.Delete(root, recursive: true);
    ...
    public async Task Updates_arc42_chapter_status_metadata()
    {
        ...
        try
        {
            ...
            var knowledge = new Arc42DevbookStore(new DevbookFolderSource(store));
    ...
    {
        var document = DevbookMarkdownParser.Parse(".arc42/08-crosscutting-concepts.md", "# Concepts\n\nA **safe** [link](https://example.com).\n\n- one\n- two");

        var paragraph = Assert.IsType<DevbookParagraphBlock>(document.Blocks[1]);
        Assert.Equal("A safe link.", PlainText(paragraph.Content));
        var list = Assert.IsType<DevbookListBlock>(document.Blocks[2]);

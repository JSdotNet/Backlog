...
public sealed class EntryScheduleControlsTests
{
    ...
    /// <summary>
    /// A repository-authored row gets the other four controls and no dependency
    /// control at all.
    /// <para>
    /// It came out of a committed <c>.backlog</c> file rather than the local store,
    /// so it has no id: nothing can name it in an <c>after:</c> token, and it has
    /// no id of its own to write into one either. A control over a value that
    /// cannot round-trip is a control that lies, so there is none.
    /// </para>
    /// </summary>
    [Fact]
    public async Task A_repository_row_gets_no_dependency_control()
    {
        using var host = await BacklogPaneHost.CreateWithRepositoryBacklogAsync(
            "# Roadmap planning\n\n```meta\nstatus: active\n```\n\nWhat is coming.\n");
        ...
        Assert.Empty(pane.FindAll("[data-testid='entry-action-depends']"));

        // The other four work on both paths — a committed entry can still be due
        // on a day, and the writer knows how to put it back in the file.
        Assert.Single(pane.FindAll("[data-testid='entry-action-myday']"));
        Assert.Single(pane.FindAll("[data-testid='entry-action-due']"));
        Assert.Single(pane.FindAll("[data-testid='entry-action-remind']"));
        Assert.Single(pane.FindAll("[data-testid='entry-action-repeat']"));
    }

    /// <summary>
    /// A due date set on a repository row lands in the committed file.
    /// <para>
    /// Which is the whole reason the other four controls are offered there at all.
    /// The metadata line for these rows is written back into somebody else's
    /// markdown by a different writer than the local store uses, so "the control is
    /// rendered" and "the value survives" are two different claims — and offering a
    /// control whose value did not survive would be exactly the lie the missing
    /// dependency control is missing to avoid.
    /// </para>
    /// </summary>
    [Fact]
    public async Task A_due_date_set_on_a_repository_row_reaches_the_committed_file()
    {
        using var host = await BacklogPaneHost.CreateWithRepositoryBacklogAsync(
            "# Roadmap planning\n\n```meta\nstatus: active\n```\n\nWhat is coming.\n");
        ...
        Assert.Contains(
            "`due:2026-08-21`",
            await File.ReadAllTextAsync(host.RepositoryBacklogFile!),
            StringComparison.Ordinal);
    }

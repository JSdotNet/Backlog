...
/// </para>
/// </summary>
public sealed class DevbookChapterWriteTests : IDisposable
{
    ...
    private readonly DevbookChapterWriter _writer = new();
    ...
    public async Task A_relative_path_that_climbs_out_of_the_root_is_refused()
    {
        ...
        var escaping = new DevbookChapterRef("arc42", Path.Combine(root, "area"), "../outside.md");
    ...
    public async Task An_absolute_path_dressed_as_a_relative_one_is_refused()
    {
        ...
        var escaping = new DevbookChapterRef("arc42", Path.Combine(root, "area"), outside);
    ...
    {
        var root = TempDir();
        var missing = new DevbookChapterRef("arc42", root, "gone.md");
    ...
    {
        var statuses = DevbookChapterStatus.Read(Layered);
        ...
        Assert.True(DevbookChapterStatus.Read("# Notes\n\nNo meta here.\n").IsEmpty);
    }
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "devbook-chapter-write-tests", Guid.NewGuid().ToString("N"));

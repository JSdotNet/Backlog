...
public sealed class TasksExternalChangePollingTests : IDisposable
{
    ...
    {
        var root = TempRoot();
        var settings = new TasksRefreshSettingsStore(Path.Combine(root, "refresh.json"));
    ...
    private static async Task WriteEntryAsync(TasksDesktopState state, string text)
    {
    ...
    private static TasksDesktopState State(
        string root,

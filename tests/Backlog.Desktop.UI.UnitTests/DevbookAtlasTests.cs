...
/// </summary>
public sealed class DevbookAtlasReaderTests
{
    ...
    private static DevbookAtlasGraph Read(DevbookAtlasScope? scope = null) =>
        DevbookAtlasReader.Read(scope ?? DomainScope, Graph());
    ...
    {
        var graph = DevbookAtlasReader.Read(DevbookAtlasScope.All, Graph());
    ...
    public void Each_node_wears_the_tone_its_own_folders_vocabulary_gives_it()
    {
        ...
        Assert.Equal(
            DevbookStatus.Vocabulary(DevbookFolder.Domain).SlugFor("proposed"),
            NodeFor(graph, ".domain/inbox/domain.md#capture").ToneSlug);

        Assert.Equal(
            DevbookStatus.Vocabulary(DevbookFolder.Arc42).SlugFor("active"),
    ...
    {
        var graph = DevbookAtlasReader.Read(DomainScope, Json(new { stats = new { nodes = 0 } }));
    ...
    {
        var graph = DevbookAtlasReader.Read(DomainScope, Json(new
        {
    ...
    private static DevbookAtlasNode NodeFor(DevbookAtlasGraph graph, string id) =>

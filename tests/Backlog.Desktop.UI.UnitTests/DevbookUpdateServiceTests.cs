...
/// </summary>
public sealed class DevbookUpdateServiceTests : IDisposable
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "backlog-devbook-update-tests", Guid.NewGuid().ToString("n"));

    [Fact]
    public void Devbook_kept_in_the_storage_folder_has_no_latest_version_to_be_on()
    {
    ...
    public async Task Checking_a_scope_with_no_clone_behind_it_asks_git_nothing()
    {
        ...
        Assert.Equal(DevbookUpdateAvailability.NotApplicable, state.Availability);
    ...
    public async Task A_clone_with_everything_the_remote_has_reports_up_to_date_and_offers_no_pull()
    {
        ...
        Assert.Equal(DevbookUpdateAvailability.UpToDate, state.Availability);
    ...
    public async Task A_clone_the_remote_has_moved_past_offers_a_pull_and_says_how_far_behind()
    {
        ...
        Assert.Equal(DevbookUpdateAvailability.UpdateAvailable, state.Availability);
        Assert.True(state.CanPull);
        Assert.Equal(3, state.BehindBy);
        Assert.Equal("3 behind", DevbookUpdatePresentation.BehindLabel(state));
        Assert.Equal("Pull latest", DevbookUpdatePresentation.ActionLabel(state));
    }
        ...
        bool hasLocalChanges)
    {
        ...
        Assert.Equal(DevbookUpdateAvailability.Blocked, state.Availability);
        Assert.False(state.CanPull);
        Assert.Equal("the reason git gave", state.Message);
        Assert.Null(DevbookUpdatePresentation.BehindLabel(state));
        Assert.Equal("Check now", DevbookUpdatePresentation.ActionLabel(state));
    }
    ...
    {
        Assert.Equal("Checking", DevbookUpdatePresentation.BusyLabel(DevbookUpdateState.NotChecked));
        Assert.Equal("Pulling", DevbookUpdatePresentation.BusyLabel(
            new DevbookUpdateState(DevbookUpdateAvailability.UpdateAvailable, 1, null)));
    }

    [Fact]
    public void One_commit_behind_still_reads_as_a_count_rather_than_as_a_bare_number()
    {
        var state = new DevbookUpdateState(DevbookUpdateAvailability.UpdateAvailable, 1, null);

        Assert.Equal("1 behind", DevbookUpdatePresentation.BehindLabel(state));
    }
    ...
    public void The_status_line_wears_a_different_modifier_for_each_state_worth_distinguishing()
    {
        var classes = new[]
        {
            DevbookUpdateAvailability.NotChecked,
            DevbookUpdateAvailability.UpToDate,
            DevbookUpdateAvailability.UpdateAvailable,
            DevbookUpdateAvailability.Blocked
        }.Select(availability => DevbookUpdatePresentation.StatusClass(new DevbookUpdateState(availability, 0, null))).ToArray();

        Assert.All(classes, css => Assert.StartsWith("devbook-stack__update-status", css, StringComparison.Ordinal));
    ...
    private Harness CreateHarness(string? cloneDirectory = "clone")
    {
        ...
        return new Harness(
            new DevbookUpdateService(gitHubSettings, git, folders),
            git,
            folders,
            repository.Alias,
            resolvedClone);
...
/// </summary>
internal sealed class AnnouncementCountingFolderSource : IDevbookFolderSource
{
    ...
    public DevbookFolderLocation Resolve(string key, string? repositoryAlias = null) =>
        DevbookFolderLocation.Unavailable(key, "Nothing is configured in this fixture.");

...
public sealed class BacklogDesktopStateLifetimeTests : IDisposable
{
    ...
    public async Task A_state_registered_with_the_bunit_container_is_disposed_with_the_context()
    {
        ...
        Assert.Same(state, context.Services.GetRequiredService<BacklogDesktopState>());
    ...
    private static async Task<EntryRow> WriteEntryAsync(BacklogDesktopState state, string text)
    {
        state.NewRow();
        var row = state.Rows[^1];

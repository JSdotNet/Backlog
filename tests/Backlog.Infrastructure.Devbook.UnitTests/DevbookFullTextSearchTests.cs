...
/// Devbook nothing could link to, follow, or cite.</para>
///
...
/// </summary>
public sealed class DevbookFullTextSearchTests
{
    [Fact]
    public void A_hit_names_the_chapter_it_came_from()
    {
        ...
        Assert.Equal(DevbookRetrievalCorpus.LadderPath, hit.Chapter.Path);
    ...
    public void An_excerpt_travels_with_its_address_and_is_one_line()
    {
        ...
        Assert.Equal(DevbookRetrievalCorpus.MentionPath + "#devbook-retrieval", hit.Chapter.Reference);
    ...
    public void An_excerpt_of_a_chapter_with_a_meta_fence_carries_no_fence()
    {
        ...
        Assert.Equal(DevbookRetrievalCorpus.FencedPath, hit.Chapter.Path);
    ...
    public void A_chapter_named_for_the_words_outranks_one_that_mentions_them()
    {
        ...
        Assert.Equal(DevbookRetrievalCorpus.LadderPath, hits[0].Chapter.Path);
    ...
    public void A_scope_narrows_the_answer_to_one_area()
    {
        ...
        var scoped = Search(temporary).Search("ladder", scope: DevbookRetrievalCorpus.DomainScope);
    ...
    {
        var answer = new DevbookFullTextSearch(new StubDevbookFolderSource(null)).Search("ladder");
    ...
    private static IDevbookSearch Search(TemporaryDatabase temporary) =>
        new DevbookFullTextSearch(new StubDevbookFolderSource(temporary.RootDirectory));
}

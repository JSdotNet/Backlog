...
/// <c>tools/devbook/devbook-schema.mjs</c> — <c>dimensions</c> IEEE-754
...
/// </summary>
public sealed class DevbookVectorEncodingTests
{
    /// <summary>
    ...
    public void A_vector_round_trips_through_the_blob()
    {
        ...
        var row = Assert.Single(database.Embeddings(DevbookRetrievalCorpus.Model, scope: null));
    ...
    {
        Assert.Equal(new byte[] { 0x00, 0x00, 0x80, 0x3F }, DevbookRetrievalCorpus.Encode(1f));
        Assert.Equal(8, DevbookRetrievalCorpus.Encode(1f, 2f).Length);
    }
    ...
    public void An_incomparable_pair_scores_zero(float[] left, float[] right) =>
        Assert.Equal(0d, DevbookVectorMath.Cosine(left, right));
    ...
    {
        Assert.Equal(1d, DevbookVectorMath.Cosine([1f, 0f], [1000f, 0f]), 6);
        Assert.Equal(-1d, DevbookVectorMath.Cosine([1f, 0f], [-3f, 0f]), 6);
    }
}

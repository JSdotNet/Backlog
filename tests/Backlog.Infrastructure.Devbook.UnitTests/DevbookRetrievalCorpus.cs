...
///
/// <para>Separate from <see cref="DevbookCorpus"/> rather than an extension of
...
/// </summary>
internal static class DevbookRetrievalCorpus
{
    ...
    public const string MentionPath = ".domain/devbook/features.md";
    ...
    public const string FencedPath = ".domain/devbook/naming.md";
    ...
    public static void Seed(string databasePath)
    {
        ...
        using var connection = DevbookSchemaSource.OpenForWriting(databasePath);
        ...
        Chapter(connection, MentionPath, "domain", "devbook-retrieval", "Devbook retrieval", 321,
        ...
        Chapter(connection, FencedPath, "domain", "devbook-note", "Knowledge Note", 21,
    ...
    /// <c>tools/devbook/devbook-schema.mjs</c> beside the table.</para>
    /// </summary>
        ...
        string? searchText = null)
    {
        ...
        command.Parameters.AddWithValue("$contentHash", DevbookCorpus.Sha256(path + "#" + slug));
        command.Parameters.AddWithValue("$sourceHash", DevbookCorpus.Sha256(path));
...
/// <summary>
/// The devbook-folder port, answering for one directory on disk.
///
...
/// </summary>
internal sealed class StubDevbookFolderSource(string? rootDirectory, string folderKey = ".domain") : IDevbookFolderSource
{
    ...
    public IReadOnlyList<DevbookFolderSetting> Folders(string? repositoryAlias) =>
        [new DevbookFolderSetting(folderKey, "Domain", folderKey)];

    public DevbookFolderLocation Resolve(string key, string? repositoryAlias = null) =>
        rootDirectory is null
            ? DevbookFolderLocation.Unavailable(key, "No folder is configured.")
            : new DevbookFolderLocation(
                key,
                Available: true,
                Message: null,
                RepositoryFullName: null,
                Folder: new DevbookFolderSetting(key, "Domain", key),

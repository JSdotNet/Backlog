...
/// schema is written by <c>tools/devbook/build-database.mjs</c> and read from
...
/// <c>tools/devbook/build-database.test.mjs</c> is its other half.</para>
///
/// <para>Every database below is created from the DDL read out of
/// <c>devbook-schema.mjs</c> at test time, never from a copy in C#. That is the
...
/// </summary>
public class DevbookSchemaContractTests
{
    [Fact]
    public void The_schema_version_matches_the_writers()
    {
        Assert.Equal(DevbookDatabaseSchema.Version, DevbookSchemaSource.Version);
    }
    ...
    public void The_writer_creates_every_table_the_reader_reads()
    {
        ...
        using var connection = DevbookSchemaSource.OpenForWriting(temporary.DatabaseFile);
        ...
        foreach (var table in DevbookDatabaseSchema.ReadTables)
        {
            Assert.True(
                tables.Contains(table, StringComparer.Ordinal),
                $"{DevbookSchemaSource.Path} no longer creates '{table}', which DevbookDatabase reads.");
        }
    }
    ...
    public void Every_table_the_reader_claims_round_trips_a_row()
    {
        ...
        // meta
        Assert.Equal(DevbookDatabaseSchema.Version.ToString(), database.Meta[DevbookDatabaseSchema.SchemaVersionKey]);
        Assert.Equal(seeded.GeneratedAt, database.GeneratedUtc);
        Assert.Equal(seeded.EmbeddingModel, database.EmbeddingModel);

        // node
        var node = Assert.Single(database.Nodes(), candidate => candidate.Id == DevbookCorpus.ChapterId);
        Assert.Equal("chapter", node.Type);
        Assert.Equal("Inbox", node.Label);
        Assert.Equal("domain", node.Folder);
        Assert.Equal(DevbookCorpus.ChapterPath, node.Path);
        ...
        // node_attribute
        Assert.Equal(["inbox-triage"], database.Attributes("roadmap")[DevbookCorpus.ChapterId]);

        // edge
        var edge = Assert.Single(database.Edges(), candidate => candidate.Type == "related");
        Assert.Equal(DevbookCorpus.ChapterId, edge.Source);
        Assert.Equal(DevbookCorpus.ExternalId, edge.Target);

        // outline_entry
        var outline = database.Outline(DevbookCorpus.Scope);
        ...
        Assert.Equal(DevbookCorpus.ChapterPath, file.Path);
        ...
        // chapter
        var chapter = Assert.Single(database.Chapters(DevbookCorpus.ChapterPath));
        ...
        Assert.Equal(DevbookCorpus.ChapterText, chapter.Text);
        Assert.Equal(DevbookCorpus.ContentHash, chapter.ContentHash);
        ...
        Assert.Equal(DevbookCorpus.FenceHash, artifact.FenceHash);
        ...
        // problem
        var problem = Assert.Single(database.Problems(DevbookCorpus.Scope));
        Assert.Equal("warning", problem.Severity);
        Assert.Equal(DevbookCorpus.ChapterPath, problem.Path);

...
3. `jsdotnet-project-design` is the authoritative source for design and UX guidance, including the color scheme and design tokens materialized into `.design/`.
...
6. Checked-in knowledge folders are **task-scoped local fallbacks**, not default context. Load `.arc42/`, `.domain/`, `.backlog/`, `.tech/`, or `.design/` only when the selected orchestration or specialist agent needs that knowledge, and then prefer only the relevant chapter(s) over whole-folder reads.

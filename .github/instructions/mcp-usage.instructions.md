...
6. Checked-in knowledge folders are **task-scoped local fallbacks**, not default context. Load `.arc42/`, `.domain/`, `.backlog/`, or `.tech/` only when the selected orchestration or specialist agent needs that knowledge, and then prefer only the relevant chapter(s) over whole-folder reads.

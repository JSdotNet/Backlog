...
- Never hand-edit anything under an `_meta/` folder; it is generated and no longer committed. The devbook database is the app's to build and lives outside the repository (local ADR 0015). The authored `_reading-order.json` beside each knowledge folder is the exception — nothing generates it, so it is edited by hand.

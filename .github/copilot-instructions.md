...
What is Backlog's own, and where it departs from that convention, is local ADR 0004 (`.arc42/adr/0004-knowledge-index-is-a-generated-local-database.md`): the derived knowledge layer is **one generated SQLite database**, `_meta/devbook.db`, and it is not committed. Build it with `node tools/devbook/build-database.mjs` — repo-native tooling that imports the installed generator's exported seam rather than editing it, the same arrangement as `tools/devbook/check-metadata.mjs`. A fresh clone has no database until you run it. The product reads both layouts — `.devbook/<name>` first, the root-level `.<name>` as the legacy fallback — and a repository on the `.devbook/` layout keeps its database at `.devbook/_meta/devbook.db`, which the same command writes there with that repository's own devbook generator; this repository is still on the root layout.
